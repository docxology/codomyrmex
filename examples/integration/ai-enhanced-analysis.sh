...
# Duration: ~5 minutes
# Output: Complete workflow results in examples/output/ai-enhanced-analysis/
...
OUTPUT_DIR="$PROJECT_ROOT/examples/output/ai-enhanced-analysis"
...
check_environment() {
    ...
    if [[ ! -f "$PROJECT_ROOT/pyproject.toml" ]] || [[ ! -d "$PROJECT_ROOT/src/codomyrmex" ]]; then
        log_error "Not in Codomyrmex project root. Please run from examples/integration/"
        exit 1
    fi
...
main() {
    ...
    log_info "Duration: ~5 minutes | Output: examples/output/ai-enhanced-analysis/"
