...
try:
    ...
    # SCRIPT_DIR is the directory containing this script (e.g., .../codomyrmex/documentation/)
    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    # CODOMYRMEX_DIR is the root of the 'codomyrmex' package (e.g., .../codomyrmex/)
    CODOMYRMEX_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, '..'))
    # To import 'from codomyrmex.something import ...', the directory *containing* the 'codomyrmex' package
    # (i.e., the parent of CODOMYRMEX_DIR) needs to be in sys.path.
    PATH_TO_ADD_FOR_MODULE_IMPORT = os.path.abspath(os.path.join(CODOMYRMEX_DIR, '..'))
except NameError:
    # Fallback if __file__ is not defined (e.g., interactive execution)
    # Assume CWD is the script's directory: .../codomyrmex/documentation/
    SCRIPT_DIR = os.getcwd()
    CODOMYRMEX_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, '..'))
    PATH_TO_ADD_FOR_MODULE_IMPORT = os.path.abspath(os.path.join(CODOMYRMEX_DIR, '..'))

_codomyrmex_dir_for_import_msg = CODOMYRMEX_DIR
...
try:
    ...
    setup_logging()
    logger = get_logger(__name__) # Use __name__ which will be '__main__' if script is run directly
    logger.info("Codomyrmex centralized logging initialized successfully via documentation_website.py.")
...
if logger is None:
    ...
    logger = logging.getLogger(__name__)
    logger.critical("Logger was not initialized by custom or primary fallback setups. Using emergency basic configuration.")
...
if _docs_base_path_for_construction: # if base path is not empty or just "/"
    EFFECTIVE_DOCS_URL = f"{_base_url_for_construction}/{_docs_base_path_for_construction}/"
else:
...
def check_doc_environment():
    ...
    if not (npm_ok or yarn_ok):
        logger.error("Neither npm nor yarn found. Please install npm (comes with Node.js) or yarn.")
    ...
    if npm_ok:
        ...
        logger.warning("npm not found. Some operations might require it if yarn is not preferred/available.")
    ...
    logger.info("Basic documentation environment check passed (Node.js and at least one package manager).")
    return True
...
def run_command_stream_output(command_parts, cwd):
    ...
    try:
        ...
        process = subprocess.Popen(command_parts, cwd=cwd, stdout=subprocess.PIPE, 
        ...
        if process.returncode == 0:
            ...
            logger.error(f"Command '{' '.join(command_parts)}' failed with return code {process.returncode}.")
            return False
    except FileNotFoundError:
        logger.error(f"Error: Command '{command_parts[0]}' not found. Is it installed and in PATH?")
        return False
    except Exception as e:
        logger.error(f"An error occurred while running '{' '.join(command_parts)}': {e}")
        return False
...
def start_dev_server(package_manager="npm"):
    ...
    logger.info(f"Docusaurus site should be available at {EFFECTIVE_DOCS_URL} once started.")
    ...
    try:
        ...
        logger.info(f"Try opening your browser at {EFFECTIVE_DOCS_URL} if it doesn't open automatically.")
        subprocess.run(cmd, cwd=DOCUSAURUS_ROOT_DIR, check=False)
        logger.info("Development server process finished or was stopped.")
        return True
    except FileNotFoundError:
        logger.error(f"Error: Command '{cmd[0]}' not found. Is it installed and in PATH?")
        return False
...
def build_static_site(package_manager="npm"):
    ...
    if success:
        logger.info(f"Static site built successfully in '{os.path.join(DOCUSAURUS_ROOT_DIR, 'build')}'")
    return success
...
def serve_static_site(package_manager="npm"):
    ...
    if not os.path.exists(build_dir) or not os.listdir(build_dir):
        logger.error(f"Build directory '{build_dir}' does not exist or is empty. Please build the site first using the 'build' action.")
        return False

    logger.info(f"Site will be served from '{build_dir}' and should be available at {EFFECTIVE_DOCS_URL}.")
    logger.info("This command will run until you stop it (Ctrl+C).")
    ...
    if package_manager == "yarn" and command_exists("yarn"):
        ...
        elif command_exists("npm"): # Fallback to `npm exec` if npx not found, for npm v7+
             logger.info("npx not found, trying 'npm exec docusaurus serve'. Requires npm v7+.")
             cmd = ["npm", "exec", "--", "docusaurus", "serve"]
        else: # Should not happen if npm was found by check_doc_environment and install_dependencies
            logger.error("npm found, but npx not found and 'npm exec' attempt might also fail. Cannot determine serve command.")
            return False

        if package_manager == "yarn": # This warning is if yarn was preferred but npm is used
             logger.warning("Yarn specified but not found or serve command unclear, falling back to npm/npx for serving.")
    else:
        logger.error("Neither npm nor yarn found, or cannot determine serve command. Cannot serve site.")
        return False
    ...
    try:
        ...
        logger.error(f"Error: Command for serving (e.g., 'npx', 'yarn', or 'docusaurus') not found. Is Docusaurus installed and in PATH?")
        return False
    except KeyboardInterrupt:
...
def assess_site():
    ...
    logger.info(f"Ensure a local server (dev or static) is running and accessible at this URL.")
    try:
        if not webbrowser.open(EFFECTIVE_DOCS_URL):
            logger.warning(f"webbrowser.open() returned False. Could not open {EFFECTIVE_DOCS_URL} automatically.")
            logger.info(f"Please open manually: {EFFECTIVE_DOCS_URL}")
    except Exception as e:
        logger.warning(f"Could not open web browser automatically: {e}. Please open manually: {EFFECTIVE_DOCS_URL}")
    
    print_assessment_checklist()

def main():
    parser = argparse.ArgumentParser(
        description=("Manage and assess the Codomyrmex documentation website. "
    ...
    if action_to_perform == "checkenv":
        ...
        if not install_dependencies(args.pm):
            logger.error(f"Dependency installation failed. Aborting '{DEFAULT_ACTION}'.")
            sys.exit(1)
        ...
        logger.info(f"--- '{DEFAULT_ACTION}' sequence potentially finished (server might have been stopped) ---")
