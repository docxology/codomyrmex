...
def main():
    console.print(
        Panel.fit(
            "[bold cyan]Codomyrmex System Health Diagnostic[/bold cyan]",
            border_style="cyan",
        )
    )
    ...
    # Worktrees Table
    wt_table = Table(
        title="Git Worktrees", show_header=True, header_style="bold magenta"
    )
    ...
    mem_table = Table(
        title="Agentic Long-Term Memory", show_header=True, header_style="bold green"
    )
    ...
    sys_table = Table(
        title="Multi-Agent System Telemetry",
        show_header=True,
