...
Codomyrmex is a **130 top-level module** Python development platform. PAI agents consume codomyrmex capabilities via the Model Context Protocol (MCP). The relationship is:
...
PAI is the **orchestrator**. Codomyrmex is the **toolbox**; the Colony Kernel is one
optional proposal-governance surface, not a mandatory path for every Codomyrmex or PAI
operation.
...
**Tools**: 9 static proxy tools plus **623** production `@mcp_tool` lines (150 `mcp_tools.py` files; **130** top-level packages) and **605** entries in the merged runtime manifest for the complete locked dependency profile — [docs/reference/inventory.md](docs/reference/inventory.md). Trust split is dynamic; run `/codomyrmexVerify` for the current safe vs destructive list. The Codomyrmex PAI Skill (`~/.claude/skills/Codomyrmex/SKILL.md`) surfaces these tools for MCP exposure.
...
UNTRUSTED ──/codomyrmexVerify──> VERIFIED ──/codomyrmexTrust──> TRUSTED
   │                                │                              │
   └── All tools start here         └── safe tools promoted          └── 5 destructive tools enabled
```

**Safe tools**: Auto-promoted to VERIFIED by `/codomyrmexVerify`. All read, list,
analyze, search, git status/diff, and discovery tools. (Cardinality tracks the live tool registry, not a fixed ~469.)

**Destructive tools (5)**: Require explicit `/codomyrmexTrust`:
...
├── src/codomyrmex/                 # 130 top-level Python modules
