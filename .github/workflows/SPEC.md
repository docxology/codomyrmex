...
- Gates: Ruff, Ruff format, `ty`, import layering, dependency validation, package build,
  ...
  provenance tests on Python 3.11. The complete unit/integration suite runs on
  Ubuntu and macOS; a named, blocking Windows portability contract covers native
  locking, process isolation, environment, filesystem, operating-system,
  terminal, structure, and real temporary-Git workflows. Live-provider and
  benchmark lanes are explicitly opt-in.
