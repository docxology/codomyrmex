...
- **60%** line-coverage floor in `[tool.coverage.report] fail_under` (`pyproject.toml`). Plain `uv run pytest` does not collect coverage; `make test`, CI unit jobs, and explicit `--cov=src/codomyrmex --cov-fail-under=60` apply the gate. The experimental `meme` package is omitted from `[tool.coverage.run]` measurement.
- Test-driven development practices encouraged; tests live alongside source in `tests/unit/<module>/`.
