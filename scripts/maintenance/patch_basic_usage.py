...
for folder, prompt_arg, default_prompt in [
    ...
    ("video", "--prompt", "A timelapse of clouds moving over a mountain range, cinematic, golden hour"),
    ("multimodal", "--prompt", "A photorealistic blue butterfly resting on a dandelion, morning light")
]:
...
"""
        ...
        if folder == "audio":
            ...
            content = re.sub(r'synthesize_to_file\(\s*"Hello! This is offline.*?",', 'synthesize_to_file(\n                args.text,', content, count=1, flags=re.DOTALL)
        else:
            prompt_var = "args.prompt"
            content = re.sub(r'prompt="A.*?",\n', f'prompt={prompt_var},\n', content, count=1)
