...
def add_to_table(content, header, new_row):
    ...
    if match:
        ...
        if new_row.split("|")[1].strip() not in table_content:
            ...
            content = (
                content[: match.start(1)] + new_table_content + content[match.end(1) :]
            )
    return content
...
# Remove tree_sitter
overview = re.sub(
    r"\| \*\*`tree_sitter`\*\* \| AST parsing \| Language-agnostic.*?\|\n", "", overview
)

overview = add_to_table(
    overview,
...
src_modules = sorted(
    [
        os.path.basename(p)
        for p in glob.glob("src/codomyrmex/*")
        if os.path.isdir(p)
        and not os.path.basename(p).startswith("_")
        and os.path.basename(p) != "documentation"
    ]
)
src_modules.append("documentation")  # keep the same set
src_modules = sorted(
    [m for m in src_modules if os.path.isfile(f"src/codomyrmex/{m}/__init__.py")]
)
children_list = ", ".join([f"[{m}/]({m}/AGENTS.md)" for m in src_modules])

agents = re.sub(
    r"- \*\*Children\*\* \(84 modules\):\n  - .*?\n",
...
# Remove tree_sitter
agents = re.sub(
    r"\| \[tree_sitter/\]\(tree_sitter/\) \| AST parsing and analysis \|\n", "", agents
)

agents = add_to_agents_table(
    agents,
...
agents = add_to_agents_table(
    agents,
    "Advanced Modules",
    "| [simulation/](simulation/) | General simulation framework |",
)
agents = add_to_agents_table(
    agents,
