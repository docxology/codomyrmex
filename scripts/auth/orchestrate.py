...
Thin orchestrator script providing CLI access to auth module functionality.
Calls actual module functions from codomyrmex.auth.
...
def handle_info(args):
    ...
    try:
        if getattr(args, "verbose", False):
            logger.info("Retrieving auth module information")

        info = {
            "module": "auth",
            "description": "Authentication and authorization",
            "path": getattr(auth, "__path__", ["unknown"])[0],
        }

        print_section("Auth Module Information")
...
Examples:
  %(prog)s info
    ...
    # Info command
    info_parser = subparsers.add_parser("info", help="Get auth module information")
