...
def main():
    ...
    call = MCPToolCall(tool_name="read_file", arguments={"path": "test.txt"})
    if call.tool_name == "read_file":
        print_success("  MCPToolCall functional.")

    # 2. Tool Result
    print_info("Testing MCPToolResult schema...")
    result = MCPToolResult(status="success", data={"output": "File content here"})
    if result.status == "success":
        print_success("  MCPToolResult functional.")

    # 3. Message
    print_info("Testing MCPMessage schema...")
    msg = MCPMessage(role="user", content="Hello MCP")
    if msg.role == "user":
