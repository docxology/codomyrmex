...
def run_example():
    ...
    builder.add_stage(
        "build",
        ["echo 'Building package...'", "mkdir -p dist", "touch dist/app.tar.gz"],
        dependencies=["test"],
    )
    builder.add_stage(
        "deploy",
    ...
    print(
        f"Generated workflow for {workflow_dict['name']} with {len(workflow_dict['jobs'])} jobs."
    )
    ...
    if os.path.exists("dist"):
        shutil.rmtree("dist")
    if os.path.exists(".example_artifacts"):
        shutil.rmtree(".example_artifacts")
