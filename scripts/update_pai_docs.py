#!/usr/bin/env python3
"""
Batch PAI.md Updater

Reads each module's __init__.py and README.md to generate accurate PAI.md content.
Only updates files that are identified as stubs (under 55 lines).

Usage:
    python scripts/update_pai_docs.py              # Dry run (show what would change)
    python scripts/update_pai_docs.py --apply       # Apply changes
"""
...
def extract_exports(init_path: Path) -> dict:
...
def infer_pai_phase(module_name: str, functions: list, classes: list) -> dict:
...
def main():
    ...
    for pai_path in sorted(SRC_DIR.glob("*/PAI.md")):
        ...
        current_lines = len(pai_path.read_text().splitlines())
        if current_lines > args.max_lines:
            skipped += 1
            continue
        ...
        try:
            ...
            if args.apply:
                pai_path.write_text(new_content)
    ...
    print(f"\n{'Applied' if args.apply else 'Dry run'}: {updated} updated, {skipped} skipped (already good), {errors} errors")
