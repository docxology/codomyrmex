...
def run_orchestration():
    ...
    if not check_doc_environment():
        print_warning(
            "Environment check failed or partially failed (Node.js/npm missing). Website features may not work."
        )
    else:
    ...
    audit_report_file = (
        report_dir / f"audit_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    )
    try:
    ...
    if consistency_report.passed:
        ...
        print_warning(
            f"Consistency check found {len(consistency_report.issues)} issues in {doc_module_path}"
        )
        for issue in consistency_report.issues[:5]:  # Show first 5
            print(
                f"  [{issue.severity}] {issue.file_path}:{issue.line_number} - {issue.description}"
            )
    ...
    try:
        ...
        quality_report_file = (
            report_dir / f"quality_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
