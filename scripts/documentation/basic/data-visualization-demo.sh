...
# Duration: ~3 minutes
# Output: Generated plots in scripts/output/data-visualization/
...
OUTPUT_DIR="$PROJECT_ROOT/scripts/output/data-visualization"
...
check_environment() {
    ...
    if [[ ! -f "$PROJECT_ROOT/pyproject.toml" ]] || [[ ! -d "$PROJECT_ROOT/src/codomyrmex" ]]; then
        log_error "Not in Codomyrmex project root. Please run from project root: ./scripts/examples/basic/data-visualization-demo.sh"
        exit 1
    fi
...
main() {
    ...
    log_info "Duration: ~3 minutes | Output: scripts/output/data-visualization/"
