...
def main():
    ...
    plan = deploy_manager.create_plan(
        agg_func, strategy=DeploymentStrategy.CANARY, canary_percent=33
    )
    deploy_manager.execute(plan)
    print(
        f"  Deployed '{agg_func.name}' via CANARY strategy. Status: {plan.state.value}"
    )
    ...
    print(
        f"  Metrics: {m_summary['total']} total calls, {m_summary['success_rate']}% success"
    )
