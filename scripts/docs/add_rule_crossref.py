...
def add_crossref_to_agents_md(agents_md: Path, cursorrules: Path, dry_run: bool) -> bool:
...
def add_crossref_to_cursorrules(cursorrules: Path, agents_md: Path, dry_run: bool) -> bool:
    ...
    comment = (
        f"\n\n{CURSORRULES_MARKER}\n"
        f"# See corresponding AGENTS.md: {rel_agents_md}\n"
    )
    if not dry_run:
...
def main(argv: list[str] | None = None) -> int:
    ...
    for module_dir in modules_with_agents_md():
        ...
        if add_crossref_to_agents_md(agents_md, cursorrules, args.dry_run):
            agents_updated += 1
            print(f"{'[DRY] ' if args.dry_run else ''}Updated AGENTS.md: {agents_md.relative_to(REPO_ROOT)}")
        if add_crossref_to_cursorrules(cursorrules, agents_md, args.dry_run):
            rules_updated += 1
            print(f"{'[DRY] ' if args.dry_run else ''}Updated .cursorrules: {cursorrules.relative_to(REPO_ROOT)}")
