...
# Duration: ~2 minutes
# Output: Analysis reports in scripts/output/static-analysis/
...
OUTPUT_DIR="$PROJECT_ROOT/scripts/output/static-analysis"
...
check_environment() {
    ...
    if [[ ! -f "$PROJECT_ROOT/pyproject.toml" ]] || [[ ! -d "$PROJECT_ROOT/src/codomyrmex" ]]; then
        log_error "Not in Codomyrmex project root. Please run from project root: ./scripts/examples/basic/static-analysis-demo.sh"
        exit 1
    fi
...
main() {
    ...
    log_info "Duration: ~2 minutes | Output: scripts/output/static-analysis/"
