...
def main() -> int:
    ...
    for cls in [AgentSerializer, AgentDeserializer, AgentSnapshot, Checkpoint, TransportMessage, MessageType]:
        print_info(f"  {cls.__name__}")

    # Exercise snapshot creation
    snapshot = AgentSnapshot(agent_id="test-agent", state={"counter": 42, "mode": "explore"})
    print_success(f"Snapshot created: agent_id={snapshot.agent_id}, keys={list(snapshot.state.keys())}")
