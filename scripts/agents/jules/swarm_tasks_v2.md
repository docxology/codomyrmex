...
In pyproject.toml, review the [tool.pytest] and [tool.coverage] configuration. Ensure coverage source includes all 127 modules. Verify the fail_under gate is set correctly. Audit the mutmut configuration for the v1.1.7 expansion targets.
...
Review the root README.md. Ensure badges are current, installation instructions work, and the project description accurately reflects the v1.1.7 state with 127 modules and 21,000+ tests.
...
Review src/codomyrmex/SPEC.md for accuracy. Ensure all 127 modules are listed with correct purpose statements, dependency maps, and public API summaries. Cross-reference against the actual module contents.
