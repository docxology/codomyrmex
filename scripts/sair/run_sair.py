...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    print(
        f"Budget usage: {meta['size_bytes']}/{MAX_BYTES} bytes ({meta['size_bytes'] / MAX_BYTES * 100:.1f}%)"
    )
...
def cmd_analyze(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "compare", None):
        ...
        print(
            analyze_run(
                load_json(args.run_file), verbose=getattr(args, "verbose", False)
            )
        )
    else:
...
def cmd_full(args: argparse.Namespace) -> None:
    ...
    if s.get("missed_problems"):
        ...
        if runs:
            print(
                f"   python scripts/sair/run_sair.py generate --refine-from {runs[-1]}"
            )
