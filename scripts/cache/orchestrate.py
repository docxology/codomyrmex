#!/usr/bin/env python3
"""
Cache Module Orchestrator

Thin orchestrator script providing CLI access to cache module functionality.
Calls actual module functions from codomyrmex.cache.
...
def handle_info(args):
    ...
    try:
        if getattr(args, "verbose", False):
            logger.info("Retrieving cache module information")

        info = {
            "module": "cache",
            "description": "Caching mechanisms",
            "path": getattr(cache, "__path__", ["unknown"])[0],
        }

        print_section("Cache Module Information")
...
Examples:
  %(prog)s info
    ...
    # Info command
    info_parser = subparsers.add_parser("info", help="Get cache module information")
