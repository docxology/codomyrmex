...
Thin orchestrator script providing CLI access to metrics module functionality.
Calls actual module functions from codomyrmex.metrics.
...
def handle_info(args):
    ...
    try:
        if getattr(args, "verbose", False):
            logger.info("Retrieving metrics module information")

        info = {
            "module": "metrics",
            "description": "Metrics collection and reporting",
            "path": getattr(metrics, "__path__", ["unknown"])[0],
        }

        print_section("Metrics Module Information")
...
Examples:
  %(prog)s info
    ...
    # Info command
    info_parser = subparsers.add_parser("info", help="Get metrics module information")
