# Security Documentation — README

**Version**: v1.0.6 | **Status**: Active | **Last Updated**: March 2026

## Overview

Comprehensive security documentation covering theory, digital security, physical security, cognitive security, AI safety, and trust & governance for the Codomyrmex platform.

## Contents

| File | Description |
| :--- | :--- |
| [index.md](index.md) | Security documentation index and overview |
| [security-theory.md](security-theory.md) | Foundational security theory |
| [digital-security.md](digital-security.md) | Digital security practices |
| [physical-security.md](physical-security.md) | Physical security considerations |
| [cognitive-security.md](cognitive-security.md) | Cognitive security and social engineering defense |
| [ai-safety.md](ai-safety.md) | AI safety principles and guardrails |
| [trust-and-governance.md](trust-and-governance.md) | Trust models and governance frameworks |

## Related Documentation

- [Security Reference](../reference/security.md) — Security quick reference
- [Defense Module](../modules/defense/) — Active defense implementation
- [Privacy Module](../modules/privacy/) — Privacy and mixnet routing
- [Identity Module](../modules/identity/) — Identity and verification

## Navigation

- **Parent**: [docs/](../README.md)
- **Root**: [Project Root](../../README.md)
