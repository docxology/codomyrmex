...
- **Σ₁⁰ (semi-decidable)**: "does a bug exist?" → the test suite (**34,334** collected tests, `uv run pytest src/codomyrmex/tests/ --collect-only -q --no-header --override-ini='addopts=' --import-mode=importlib`) is a partial oracle
