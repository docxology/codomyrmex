...
Codomyrmex's 545 `@mcp_tool`-decorated functions across 127 modules constitute one of the largest compositional tool surfaces in any open-source AI development platform. This essay examines how this tool ecosystem relates to the AGI requirement for autonomous agency.
...
Formally, discovery computes: δ: **Mod** → Set(T) where T is the set of tool descriptors. Each descriptor is a typed signature: `t: (τ₁ × τ₂ × ... × τₙ) → τᵣ`. The MCP bridge exposes the full tool set |T| = 545 to agents.

This implements Patil et al.'s (2023) "tool retrieval" — the first bottleneck in tool-augmented AI. The information-theoretic cost: selecting from |T| tools requires ≥ log₂(545) ≈ 9.09 bits of decision per invocation. Reducing this cost is the function of Phase 2.

### Phase 2: Selection — The Bandits Problem

Given task τ, the agent must select tools from the 545 available. This is a **contextual multi-armed bandit** problem (Li et al., 2010): each tool is an arm, the context is the task description, and the reward is task completion quality.
...
Mialon et al.'s (2023) insight: **the boundary of agency is the boundary of tool access**. An agent with 545 tools spanning code editing, security scanning, database management, LLM inference, and container orchestration has an *agency volume* proportional to the product of its capability dimensions.
...
graph TD
    subgraph LATTICE["Agency Lattice 2^T"]
        TOP["⊤ = All 545 tools<br/><i>full autonomy</i>"]
