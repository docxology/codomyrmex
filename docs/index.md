...
- **Zero-Mock testing** — all tests use real functional verification (**34,422** tests collected with `uv run pytest src/codomyrmex/tests/ --collect-only -q --no-header --override-ini='addopts=' --import-mode=importlib`; see [inventory](reference/inventory.md))
