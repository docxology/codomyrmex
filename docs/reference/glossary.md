...
Eligible MCP tools discovered at runtime via `@mcp_tool` decorators across the package tree. Discovery and registration filters mean that physical decorator lines are not a one-to-one runtime count: the current authoritative inventory reports 151 non-test `mcp_tools.py` files, 627 production decorators, and 612 merged runtime-manifest tools in the complete locked dependency profile. Other transport profiles are enumerated at server startup rather than copied here as a second static count.
