...
The Colony Kernel applies this model to callers that opt into its advisory proposal path,
...
Strict mode is intentionally narrower than a repository-wide enforcement claim. The
governed scope map currently names `patch_file`, `run_tests`, and `archive_module` target
prefixes. An `EXECUTE` decision for a registered target receives an Ed25519-signed,
single-use capability. A registered real executor consumes it atomically and returns one
signed receipt; only that receipt can authorize an `attested_execution` outcome. Unlinked
reports are quarantined and do not update trust, budgets, or `FAILURE` pressure. Role
labels remain descriptive, and paths outside the scope map are explicit bypasses rather
than silently governed actions.
...
**Key types:**
- `SignalType` enum: `FAILURE`, `POLICY_REJECTION`, `SUCCESS`, `RISK`, `NEED`, `DEPENDENCY`, `HUMAN_PRIORITY`
...
**Purpose:** An advisory proposal-evaluation boundary. Callers may submit proposals for a weighted additive score with hard overrides for budget, SANDBOX role, trust floor, and CRITICAL falsification; downstream execution remains outside this module.
