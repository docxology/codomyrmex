...
| Subsystem | File | Status | Test/coverage evidence |
|-----------|------|--------|-------|----------|
| Shared Contract | `models.py` | Implemented | Covered by the live scoped suite |
| PheromoneStore | `pheromone_store.py` | Implemented | Covered by the live scoped suite |
| ResourceLedger | `resource_ledger.py` | Implemented | Covered by the live scoped suite |
| ActuationGate | `actuation_gate.py` | Implemented | Covered by the live scoped suite |
...
Codomyrmex is an artificial-ecology design hypothesis in which proposal evaluation uses pressure, consequence, and memory in addition to explicit policy. The Colony Kernel is the control plane that implements the currently tested portion of this loop.

A real ant colony has no software permission gate; the analogy is useful only for
environment-mediated coordination. In this implementation, signal pressure is one
input to an explicit deterministic policy. It does not create authority, retire agents,
or establish that a caller's report is true.

The Colony Kernel applies a bounded version of this model to proposals submitted to its
control plane:

- **Pheromone signals** supplement an explicit gate policy. Caller-reported failures can
  weaken later same-target proposals, while caller-reported successes can reinforce the
  stored history; these reports are not independently attested by the current API.
- **Resource budgets** make estimated costs explicit. The ledger reports affordability during proposal evaluation; callers must record actual cost after an action because the kernel does not execute or enforce the action.
- **Roles** are deterministic labels derived from consequence history, not a complete
  authorization matrix. Higher labels do not currently grant distinct action powers.
- **Pruning** nominates stale or redundant module locations for review. It does not delete
  modules autonomously.
- **Falsification** is a first-class proposal check. The worker evaluates configured
  vectors before ordinary scoring; CRITICAL findings force refusal, while non-critical
  findings remain evidence in the returned decision and do not prove a plan is safe.
...
For proposals that enter `ColonyKernel.propose_action`, this is the implemented control
flow. The kernel does not execute the proposed action, attest the outcome, or mediate
every action taken by other Codomyrmex modules.
...
**Purpose:** The proposal-evaluation path for callers that submit work to
`ColonyKernel.propose_action`. It is not the only path to execution and does not
interpose on unrelated agent actions. The gate computes a weighted additive score with
hard overrides for budget, SANDBOX role, trust floor, and CRITICAL falsification.
...
### 6. `role_adapter.py` — Deterministic Role Assignment

**Purpose:** Roles are derived deterministically from the recorded proposal count and trust score. The labels are descriptive policy inputs; they are not evidence of emergent cognition or a complete authorization system.

**Key class:** `RoleAdapter`

**Role definitions:**

| Role | Promotion condition | Label semantics |
|------|--------------------|-------------------|
| `SANDBOX` | fewer than 3 proposals, or trust < 0.20 | Bootstrap label; the gate refuses SANDBOX proposals |
| `REPAIR_ANT` | trust >= 0.20 and proposals >= 3 | Repair-oriented label; no action permission is granted by the label alone |
| `MEMORY_ANT` | trust >= 0.35 and proposals >= 3 | Memory-oriented label; no action permission is granted by the label alone |
| `DISPATCHER` | trust >= 0.50 and proposals >= 3 | Coordination-oriented label; no action permission is granted by the label alone |
| `GUARD_ANT` | trust >= 0.70 and proposals >= 3 | Review-oriented label; no action permission is granted by the label alone |
...
- Role history is not persisted by the current adapter; callers that need an audit trail must record profile snapshots or outcome events separately.
...
- The current daemon has no archive/delete operation. Candidates are reported for separate operator review and an independently authorized maintenance action.
...
- `PruningDaemon.scan` identifies configured stale or duplicate candidates for review; any confidence values are heuristic outputs, not calibrated probabilities
- No pruning action is performed by the daemon; archival or deletion requires a separate, explicitly authorized maintenance path
