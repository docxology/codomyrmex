...
**Collected tests (repo-wide):** **34,334** — `uv run pytest src/codomyrmex/tests/ --collect-only -q --no-header --override-ini='addopts=' --import-mode=importlib` from the repository root after `uv sync --all-extras --dev`. Count varies with optional extras and discovery paths.
...
|----------|----------|--------------------|--------|
| **All collected** | under `src/codomyrmex/` (`tests/` + `tests/**`) | **34,334** | Single source of truth: scoped `pytest --collect-only` (with CI-parity extras) |
