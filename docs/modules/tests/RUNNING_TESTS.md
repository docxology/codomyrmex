...
**Collected tests (repo-wide):** **35,782** — measured with `uv run --locked --all-groups --all-extras python scripts/doc_inventory.py --pytest` from the repository root after `uv sync --locked --all-groups --all-extras`. Count varies with optional extras and discovery paths; refresh the snapshot in [docs/reference/inventory.md](../../reference/inventory.md) after test discovery changes.
...
| ------------------------ | -------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------- |
| **All collected**        | under `tests/` (`tests/` + `tests/**`) | **35,782**         | Complete locked dependency profile; single source of truth: `uv run python scripts/doc_inventory.py --pytest` |
