...
**Canonical copy:** [tests/RUNNING_TESTS.md](../../../tests/RUNNING_TESTS.md) — keep this module doc in sync when changing commands or counts.
...
**Collected tests (repo-wide):** **36,024** — measured with `uv run --locked --all-groups --all-extras python scripts/doc_inventory.py --pytest` from the repository root after `uv sync --locked --all-groups --all-extras`. Count varies with optional extras and discovery paths; refresh the snapshot in [docs/reference/inventory.md](../../reference/inventory.md) after test discovery changes.
...
| ------------------------ | -------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------- |
| **All collected**        | under `tests/` (`tests/` + `tests/**`) | **36,024**         | Complete locked dependency profile; single source of truth: `uv run --locked --all-groups --all-extras python scripts/doc_inventory.py --pytest` |
...
# Explicitly opt into live local-Ollama tests (normally skipped)
RUN_LIVE_OLLAMA=1 uv run pytest -m requires_ollama tests/
```
