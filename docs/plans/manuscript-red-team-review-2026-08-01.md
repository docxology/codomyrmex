...
| RT-10 | Low | A Chrome pass at a 1280 px viewport exposed max-content overflow from the large cover heading in the two-column HTML cover grid. | Readers at that viewport could encounter horizontal scrolling and clipped cover layout. | Added grid-item shrinking and overflow wrapping to the manuscript CSS; rerender and browser checks must include the corrected viewport. | No overflow remains after the CSS fix at the tested viewport; responsive behavior outside tested widths remains bounded by the HTML QA scope. |
| RT-11 | Medium | The compiler recorded `pdfinfo` and veraPDF output but only failed closed on qpdf, so a requested tagged/PDF-UA artifact could theoretically be emitted with missing or failing conformance evidence. | A stale or nonconforming PDF could be mistaken for a validated deliverable when host validators were unavailable or rejected it. | The compiler and integrity validator now map the requested PDF standard to its veraPDF profile, require `Tagged: yes` and `Suspects: no`, parse compliant veraPDF results, require schema-v2 receipts for both rendered PDFs, and record an overall receipt pass. | Current content and distribution PDFs must be regenerated through the enforced gate; artifact-specific conformance still does not establish universal reader usability. |
