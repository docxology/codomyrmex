# Agent Documentation — Specification

**Version**: v1.1.4 | **Status**: Active | **Last Updated**: February 2026

## Purpose

Defines the structure and behavioral standards for agent coordination documentation within the Codomyrmex ecosystem.

## Architectural Constraints

- **Modularity**: Agent rules must maintain strict modular boundaries — each rule file addresses a specific concern.
- **Real Execution**: Rules must guarantee executable, verifiable behavior without reliance on stubbed or mocked data.
- **Hierarchy**: Agent directives follow a clear hierarchy: root `AGENTS.md` → `docs/AGENTS.md` → `docs/agents/` rules.

## Navigation

- **README**: [README.md](README.md)
- **AGENTS**: [AGENTS.md](AGENTS.md)
- **Parent**: [docs/](../README.md)
