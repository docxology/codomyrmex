# References {#sec:references .unnumbered}

::: {#refs}
:::

<!-- 
New references added by expanded sections (02_theory.md, 08_active_inference.md,
appendix_design_rationale.md, expanded 07_scope_and_related_work.md, 
expanded 04_conclusion.md, expanded 03_results.md).

These are formatted as pandoc-citeproc bibtex-compatible keys. The bibliography
entries should be added to the project's .bib file (refs.bib or equivalent).

=== Mathematical Foundations (sec:theory) ===

@rudin1991analysis
  Rudin, W. (1991). Functional Analysis (2nd ed.). McGraw-Hill.

@letac1986systemes
  Letac, G. (1986). A contraction principle for certain Markov chains and its applications.
  Contemporary Mathematics, 50, 263–273.

@robbins1951stochastic
  Robbins, H., & Monro, S. (1951). A stochastic approximation method.
  Annals of Mathematical Statistics, 22(3), 400–407.

@kushner2003stochastic
  Kushner, H. J., & Yin, G. G. (2003). Stochastic Approximation and Recursive Algorithms
  and Applications (2nd ed.). Springer.

@dwork2014algorithmic
  Dwork, C., & Roth, A. (2014). The Algorithmic Foundations of Differential Privacy.
  Foundations and Trends in Theoretical Computer Science, 9(3–4), 211–407.

@near2021programming
  Near, J. P., & Abuah, C. (2021). Programming Differential Privacy. Book.
  https://programming-dp.com/

=== Active Inference (sec:active-inference) ===

@friston2017active
  Friston, K. J., Rosch, R., Parr, T., Price, C., & Bowman, H. (2017).
  Active inference: a process theory. Neural Computation, 29(1), 1–49.

@jordan1999introduction
  Jordan, M. I., Ghahramani, Z., Jaakkola, T. S., & Saul, L. K. (1999).
  An introduction to variational methods for graphical models.
  Machine Learning, 37(2), 183–233.

@amari1998natural
  Amari, S. I. (1998). Natural gradient works efficiently in learning.
  Neural Computation, 10(2), 251–276.

@ramstead2019multiscale
  Ramstead, M. J. D., Badcock, P. B., & Friston, K. J. (2019).
  Answering Schrödinger's question: A free-energy formulation.
  Physics of Life Reviews, 24, 1–16.

@pearl1988probabilistic
  Pearl, J. (1988). Probabilistic Reasoning in Intelligent Systems. Morgan Kaufmann.

@friston2013life
  Friston, K. (2013). Life as we know it. Journal of the Royal Society Interface,
  10(86), 20130475.

=== Information-Theoretic Security and Mechanism Design (sec:infosec-dp, sec:mechanism-design) ===

@bonawitz2017practical
  Bonawitz, K., Ivanov, V., Kreuter, B., Marcedone, A., McMahan, H. B., Patel, S., ...
  & Seth, K. (2017). Practical secure aggregation for privacy-preserving machine learning.
  CCS 2017, 1175–1191.

@mohassel2017secureml
  Mohassel, P., & Zhang, Y. (2017). SecureML: A system for scalable privacy-preserving
  machine learning. IEEE S&P 2017, 19–38.

@roughgarden2016cs269i
  Roughgarden, T. (2016). Twenty lectures on algorithmic game theory.
  Cambridge University Press.

@myerson1979incentivecompatibility
  Myerson, R. B. (1979). Incentive compatibility and the bargaining problem.
  Econometrica, 47(1), 61–73.

@surowiecki2004wisdom
  Surowiecki, J. (2004). The Wisdom of Crowds. Doubleday.

@condorcet1785essai
  Condorcet, M. J. A. N. de C. (1785). Essai sur l'application de l'analyse à la
  probabilité des décisions rendues à la pluralité des voix. Imprimerie Royale, Paris.

@simon1962architecture
  Simon, H. A. (1962). The architecture of complexity. Proceedings of the American
  Philosophical Society, 106(6), 467–482.

@rumelhart1986parallel
  Rumelhart, D. E., McClelland, J. L., & the PDP Research Group (Eds.). (1986).
  Parallel Distributed Processing: Explorations in the Microstructure of Cognition.
  MIT Press.

=== Design Rationale (appendix_design_rationale.md) ===

@tversky1991lossbias
  Tversky, A., & Kahneman, D. (1991). Loss aversion in riskless choice: A reference-
  dependent model. Quarterly Journal of Economics, 106(4), 1039–1061.

-->
