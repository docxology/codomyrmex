...
$$
g = w_b \cdot b + w_\rho \cdot \rho(h) + w_u \cdot u + w_c \cdot c
...
| `{#eq:trust_delta}` | $\Delta_n = \delta_\text{test}(n) + \delta_\text{repair}\mathbf{1}_\text{repair}(n) + f_n \cdot \delta_\text{human}$ | `02_methodology.md`, `11_supplemental_notation.md` |
...
```markdown
![Caption text.](figures/filename.png){#fig:label width=80% alt="Concise structural alternative." aria-describedby="fig:label-description"}
<div id="fig:label-description" class="figure-long-description">Extended description of layout, relationships, encodings, and claim limits.</div>

<!-- Reference in text -->
[@fig:label] shows the architecture.
```

Figure source PNG files live in `output/figures/` and are generated by `scripts/generate_manuscript_figures.py`. The generator reads `output/data/manuscript_variables.json`, `docs/manuscript/config.yaml`, and `config/colony_kernel/roles.yaml` before drawing the assets, then stamps each figure with the manuscript version, config hash, and generation date. Captions, concise alternatives, extended descriptions, labels, widths, filenames, and evidence classes are defined once under `config.yaml` and injected as `FIGURE_*` tokens. Captions state evidence and interpretation; alternatives describe structure without duplicating the caption; extended descriptions preserve layout, relationships, redundant encodings, and claim limits. Re-run variable generation and the figure script together before rendering.
...
`output/figures/figure_registry.json` records the generated filenames, captions,
concise alternatives, extended descriptions, evidence classes, byte sizes, SHA-256
hashes, and source-current provenance (commit, dirty state, and Colony Kernel /
...
| `{{CONFIG_RELEASE_TAG}}` | CONFIG | generated transmission bookends |
...
[`preamble.md`](https://github.com/docxology/codomyrmex/blob/main/docs/manuscript/preamble.md)
contains the LaTeX packages that Pandoc consumes:
...
- [`preamble.md`](https://github.com/docxology/codomyrmex/blob/main/docs/manuscript/preamble.md) — Active LaTeX preamble
