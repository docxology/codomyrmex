...
| `02_methodology.md` | Control-plane design, linear field, gate, trust, labels, pruning, falsification, supporting modules (config_loader, invariants, reference, formal, attestation, replay, research/), and feedback sequence | `CONFIG_BASE_EVAPORATION_RATE`, `CONFIG_PARAMETER_STATUS_NOTE`, `CONFIG_PHEROMONE_RETENTION_FAST_PCT`, `CONFIG_PHEROMONE_RETENTION_NORMAL_PCT`, `CONFIG_PHEROMONE_RETENTION_SLOW_PCT` | `fig:architecture`, `fig:falsification_vectors`, `fig:pressure_loop` |
