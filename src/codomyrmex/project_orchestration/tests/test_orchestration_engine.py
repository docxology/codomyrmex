...
from codomyrmex.project_orchestration.workflow_manager import WorkflowStep, WorkflowStatus
from codomyrmex.project_orchestration.task_orchestrator import Task, TaskStatus, TaskPriority
from codomyrmex.project_orchestration.project_manager import Project, ProjectStatus, ProjectType
from codomyrmex.project_orchestration.resource_manager import Resource, ResourceType, ResourceStatus
...
class TestOrchestrationEngine:
    ...
    def test_list_sessions_by_status(self, orchestration_engine):
        ...
        active_sessions = orchestration_engine.list_sessions(status=SessionStatus.ACTIVE)
        assert len(active_sessions) == 1
        assert active_sessions[0].name == "Active Session"
        
        # List completed sessions
        completed_sessions = orchestration_engine.list_sessions(status=SessionStatus.COMPLETED)
    ...
    def test_add_task_to_session(self, orchestration_engine):
        ...
        result = orchestration_engine.add_task_to_session(
            session.session_id,
            task
        )
...
class TestOrchestrationEngineIntegration:
    ...
    def test_complete_workflow_session(self, temp_dir):
        ...
        steps = [
            ...
            WorkflowStep(name="visualize", module="data_visualization", action="create_chart", dependencies=["analyze"])
        ]
        ...
        task2 = Task(name="task2", module="module2", action="action2", dependencies=["task1"])
