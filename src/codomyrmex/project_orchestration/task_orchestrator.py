...
class Task:
    ...
    def __post_init__(self):
        ...
        if not self.name:
            self.name = f"{self.module}.{self.action}" if self.module and self.action else self.id
...
class TaskOrchestrator:
            ...
            try:
                ...
                if result.success:
                    ...
                    if task.can_retry():
                        ...
                        logger.error(f"Task failed: {task.name} ({task.id}) - {result.error_message}")
    ...
    def get_execution_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'total_execution_time': sum(t.execution_time or 0 for t in tasks if t.execution_time),
            'average_execution_time': 0
        }
        
        completed_tasks = [t for t in tasks if t.execution_time]
        if completed_tasks:
            stats['average_execution_time'] = stats['total_execution_time'] / len(completed_tasks)
    ...
    def wait_for_completion(self, timeout: Optional[float] = None) -> bool:
        ...
        while True:
            pending_tasks = [t for t in self.list_tasks() 
