...
class ResourceManager:
    ...
    def __init__(self, config_file: Optional[str] = None):
        ...
        self.cleanup_thread = threading.Thread(target=self._cleanup_expired_allocations, daemon=True)
    ...
    def remove_resource(self, resource_id: str) -> bool:
        ...
        with self.lock:
            ...
            if resource.current_users:
                logger.error(f"Cannot remove resource {resource_id} - currently in use by {resource.current_users}")
                return False
            ...
            to_remove = [aid for aid, alloc in self.allocations.items() if alloc.resource_id == resource_id]
    ...
    def list_resources(self, resource_type: Optional[ResourceType] = None, 
                      status: Optional[ResourceStatus] = None) -> List[Resource]:
    ...
    def allocate_resources(self, user_id: str, requirements: Dict[str, Dict[str, Any]], 
                         timeout: Optional[int] = None) -> Optional[Dict[str, str]]:
        """
        ...
        try:
            with self.lock:
                ...
                for req_key, req_spec in requirements.items():
                    resource_type = ResourceType(req_key) if isinstance(req_key, str) else req_key
                    ...
                    if not suitable_resources:
                        ...
                        logger.warning(f"No suitable {resource_type.value} resource found for user {user_id}")
                    ...
                    if best_resource.allocate(req_spec, user_id):
                        ...
                        alloc_id = f"{user_id}_{best_resource.id}_{len(self.allocations)}"
                        allocation = ResourceAllocation(
                            id=alloc_id,
                            resource_id=best_resource.id,
                            user_id=user_id,
                            allocated=req_spec,
                            expires_at=datetime.now(timezone.utc) + timezone.utc.utcoffset(None) if timeout else None
                        )
                        self.allocations[alloc_id] = allocation
                        
                        logger.debug(f"Allocated {resource_type.value} resource {best_resource.id} to {user_id}")
                    else:
                        # Allocation failed
                        self._rollback_allocations(allocated_resources, user_id)
                        return None
                
                logger.info(f"Successfully allocated {len(allocations)} resources to {user_id}")
                return allocations
    ...
    def deallocate_resources(self, user_id: str, allocation_ids: Optional[List[str]] = None) -> bool:
        """
        ...
        with self.lock:
            ...
            if allocation_ids:
                ...
                to_release = [aid for aid, alloc in self.allocations.items() if alloc.user_id == user_id]
        ...
        with self.lock:
            if resource_id:
                ...
                for resource_type in ResourceType:
                    ...
                    usage['resources_by_type'][resource_type.value] = len(type_resources)
                    
                    if type_resources:
                        avg_utilization = sum(
                            sum(r.get_utilization().values()) / max(1, len(r.get_utilization()))
                            for r in type_resources
                        ) / len(type_resources)
                        usage['utilization_summary'][resource_type.value] = round(avg_utilization, 2)
            ...
            try:
                with self.lock:
                    ...
                    for alloc_id in expired_allocations:
                        ...
                        if resource:
                            resource.deallocate(allocation.allocated, allocation.user_id)
        ...
        with self.lock:
            ...
            for resource in self.resources.values():
                ...
                if any(util > 90 for util in utilization.values()):
                    ...
                    health['issues'].append(f"Resource {resource.name} is over 90% utilized")
                
                if resource.status != ResourceStatus.AVAILABLE:
                    resource_health['healthy'] = False
                    health['issues'].append(f"Resource {resource.name} is not available")
                
                health['resources'][resource.id] = resource_health
            
            if health['issues']:
                health['overall_status'] = 'degraded' if len(health['issues']) < 5 else 'unhealthy'
