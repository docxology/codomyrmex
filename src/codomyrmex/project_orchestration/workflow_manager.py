            ...
            for step in steps:
                for dep in step.dependencies:
                    if dep not in step_names:
                        self.logger.warning(f"Step '{step.name}' depends on unknown step '{dep}'")
        ...
        try:
            ...
            while remaining_steps:
                ...
                if not ready_steps:
                    ...
                    raise ValueError(f"Circular dependency detected in workflow steps: {remaining_names}")
                ...
                for step in ready_steps:
                    try:
                        ...
                        error_msg = f"Step {step.name} execution error: {str(step_error)}"
                        execution.errors.append(error_msg)
                        self.logger.error(error_msg)
                        remaining_steps.remove(step)
    ...
    def _load_workflows(self) -> None:
        ...
        try:
            if not self.config_dir.exists():
                self.logger.info(f"Configuration directory {self.config_dir} does not exist, skipping workflow loading")
                return
            ...
            for workflow_file in self.config_dir.glob("*.json"):
                try:
                    ...
                    if 'name' not in workflow_data or 'steps' not in workflow_data:
                        self.logger.warning(f"Invalid workflow file {workflow_file}: missing required fields")
                        continue
                    ...
                    self.logger.error(f"Invalid JSON in workflow file {workflow_file}: {e}")
                except KeyError as e:
                    self.logger.error(f"Missing required field in workflow file {workflow_file}: {e}")
                except Exception as e:
