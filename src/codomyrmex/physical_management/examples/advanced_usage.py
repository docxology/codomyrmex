...
from codomyrmex.physical_management import (
    PhysicalObjectManager, ObjectType, ObjectStatus, MaterialType, EventType,
    PhysicsSimulator, Vector3D, ForceField,
    SensorManager, SensorType, SensorReading,
...
def example_smart_factory_monitoring():
    ...
    print(f"Network connectivity: {network_metrics['average_degree']:.2f} avg connections per object")
...
def example_iot_sensor_network():
    ...
    temp_stream = analytics.create_stream("temperature", buffer_size=5000, window_duration=30.0)
    humidity_stream = analytics.create_stream("humidity", buffer_size=5000, window_duration=30.0)
    pressure_stream = analytics.create_stream("pressure", buffer_size=5000, window_duration=30.0)
    ...
    for minute in range(10):  # Simulate 10 minutes
        ...
        for second in range(60):  # One reading per second
            ...
            sensor_manager.add_reading(SensorReading("sensor_001", SensorType.TEMPERATURE, temp, "°C"))
            sensor_manager.add_reading(SensorReading("sensor_002", SensorType.HUMIDITY, humidity, "%"))
            sensor_manager.add_reading(SensorReading("sensor_003", SensorType.PRESSURE, pressure, "hPa"))
    ...
    if temp_data:
        prediction = predictor.predict_linear_trend(temp_data, 60.0)  # Predict 1 minute ahead
        if prediction:
...
def example_physics_simulation_network():
    ...
    for step in range(600):  # 10 seconds of simulation
        ...
        if step % 10 == 0:
            ...
            analytics.add_data("kinetic_energy", stats["total_kinetic_energy"], "simulator")
            analytics.add_data("potential_energy", stats["total_potential_energy"], "simulator")
            ...
            for obj in pendulum_objects:
                ...
                if state:
                    ...
                    manager.update_object_location(obj.id, new_pos.x, new_pos.y, new_pos.z)
    ...
    print(f"Network maintained {final_network_metrics['total_connections']} connections")
...
def example_thermal_management():
    ...
    for sink in heat_sinks:
        ...
        temp_streams[stream_id] = analytics.create_stream(stream_id, window_duration=20.0)
    ...
    for minute in range(5):  # 5 minutes of thermal simulation
        for second in range(60):
            ...
            for sink in heat_sinks:
                ...
                thermal_diffusion = math.exp(-distance / 10.0) * math.exp(-current_time / 300.0)
                ...
                # Add to analytics
                stream_id = f"temp_{sink.id}"
                analytics.add_data(stream_id, new_temp - 273.15, sink.id)  # Convert to Celsius
...
def example_autonomous_vehicle_fleet():
    ...
    print(f"Fleet setup: {len(vehicles)} vehicles, {len(charging_stations)} charging stations, {len(waypoints)} waypoints")
    ...
    low_charge_vehicles = [v for v in vehicles if v.properties.get("charge_level", 100) < 30]
    ...
    for vehicle in low_charge_vehicles:
        ...
        if nearest_station and nearest_station.has_tag("charging"):
            ...
            if path:
                print(f"Route found for {vehicle.name}: {len(path)} waypoints to {nearest_station.name}")
            else:
...
def example_distributed_sensor_calibration():
    ...
    for sensor_id in sensor_ids:
        ...
        print(f"  {sensor_id}: {status_indicator} {health['status']}, {drift_indicator} {drift['status']}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Summary statistics
        total_objects = sum(len(m.registry.objects) for m in [factory_manager, physics_manager] if m)
