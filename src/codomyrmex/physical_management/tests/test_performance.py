...
from codomyrmex.physical_management import (
    ...
    MaterialType, SpatialIndex, ObjectRegistry, EventType
)
...
class PerformanceMetrics:
    ...
    def get_statistics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "stdev": statistics.stdev(self.execution_times) if len(self.execution_times) > 1 else 0.0
        }
...
class TestObjectCreationPerformance:
    ...
    def test_bulk_object_creation_performance(self):
        ...
        assert avg_time_per_object < 0.001, f"Object creation too slow: {avg_time_per_object:.4f}s per object"
        
        # Verify all objects were created
        assert len(manager.registry.objects) == num_objects
        
        print(f"Created {num_objects} objects in {total_time:.3f}s ({avg_time_per_object*1000:.2f}ms per object)")
    ...
    def test_object_lookup_performance(self):
        ...
        lookup_ids = [f"obj_{i}" for i in range(0, num_objects, 100)]  # Every 100th object
        ...
        assert avg_time_per_lookup < 0.0001, f"Object lookup too slow: {avg_time_per_lookup:.6f}s per lookup"
        
        print(f"Performed {len(lookup_ids)} lookups in {total_time:.4f}s ({avg_time_per_lookup*1000000:.1f}μs per lookup)")
...
class TestSpatialIndexPerformance:
    ...
    def test_spatial_index_insertion_performance(self):
        ...
        assert avg_time_per_insertion < 0.0001, f"Spatial index insertion too slow: {avg_time_per_insertion:.6f}s per insertion"
        
        print(f"Inserted {num_objects} objects into spatial index in {total_time:.3f}s ({avg_time_per_insertion*1000000:.1f}μs per insertion)")
    ...
    def test_spatial_query_performance(self):
        ...
        # Assert reasonable performance
        assert avg_time_per_query < 0.01, f"Spatial query too slow: {avg_time_per_query:.4f}s per query"
        
        print(f"Performed {num_queries} spatial queries in {total_time:.3f}s ({avg_time_per_query*1000:.2f}ms per query)")
...
class TestDistanceCalculationPerformance:
    ...
    def test_distance_calculation_performance(self):
        ...
        # Assert reasonable performance
        assert avg_time_per_calculation < 0.00001, f"Distance calculation too slow: {avg_time_per_calculation:.8f}s per calculation"
        
        print(f"Performed {num_calculations} distance calculations in {total_time:.4f}s ({avg_time_per_calculation*1000000:.1f}μs per calculation)")
...
class TestCollisionDetectionPerformance:
    ...
    def test_collision_detection_performance(self):
        ...
        assert total_time < 5.0, f"Collision detection too slow: {total_time:.3f}s for {num_objects} objects"
        
        print(f"Collision detection for {num_objects} objects completed in {total_time:.3f}s, found {len(collisions)} collisions")
...
class TestNetworkAnalysisPerformance:
    ...
    def test_network_topology_performance(self):
        ...
        # Assert reasonable performance
        assert topology_time < 1.0, f"Network topology generation too slow: {topology_time:.3f}s"
        assert metrics_time < 2.0, f"Network metrics calculation too slow: {metrics_time:.3f}s"
        
        print(f"Network topology for {num_objects} objects: {topology_time:.3f}s")
        print(f"Network metrics calculation: {metrics_time:.3f}s")
        print(f"Network has {metrics['total_connections']} connections, density: {metrics['density']:.4f}")
...
class TestEventSystemPerformance:
    ...
    def test_event_emission_performance(self):
        ...
        print(f"Event emission for {num_objects} objects: {total_time:.3f}s ({avg_time_per_event*1000:.1f}ms per event)")
...
class TestMemoryUsage:
    ...
    def test_registry_scaling(self):
        ...
        for scale in scales:
            ...
            print(f"Registry with {scale} objects: {total_size} bytes ({total_size/scale:.1f} bytes per object)")
        ...
        if len(memory_usage) >= 2:
            ...
            assert ratio / scale_ratio < 2.0, f"Memory usage scaling poorly: {ratio/scale_ratio:.2f}x"
...
class TestStressTests:
    ...
    def test_large_scale_operations(self):
        ...
        start_time = time.time()
        for i in range(100):
            nearby = manager.get_nearby_objects(
                x=i * 10,
                y=i * 10,
                z=0,
                radius=50
