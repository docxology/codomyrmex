...
try:
    from codomyrmex.git_operations import (
        initialize_git_repository, add_files, commit_changes,
        create_branch, switch_branch, get_status, get_commit_history
    )
    from codomyrmex.git_operations.visualization_integration import (
        create_git_analysis_report, visualize_git_branches,
        visualize_commit_activity, get_repository_metadata
    )
...
""")
    ...
    def test_comprehensive_git_visualization_report(self):
        ...
        assert success_count >= 3, f"Expected at least 3 successful visualizations, got {success_count}"
        ...
        assert len(created_files) >= 4, f"Expected at least 4 files to be created, got {created_files}"
    ...
    def test_repository_metadata_extraction(self):
        ...
        assert metadata["current_branch"] in ["main", "master"]  # Could be either depending on Git config
        ...
        assert structure_stats["files"] >= 3  # README.md, main.py, auth.py, test_auth.py, config.json
    ...
    def test_workflow_diagram_generation(self):
        ...
        for workflow_type in workflow_types:
            from codomyrmex.git_operations.visualization_integration import create_git_workflow_diagram
            
            result = create_git_workflow_diagram(
                workflow_type=workflow_type,
                output_path=os.path.join(self.output_dir, f"workflow_{workflow_type}.mmd"),
                title=f"{workflow_type.replace('_', ' ').title()} Workflow"
            )
            
            assert result["success"] is True, f"Failed to create {workflow_type} workflow"
    ...
    def test_mermaid_content_validation(self):
        ...
        branch_content = visualizer.visualize_git_tree_mermaid(
            repository_path=self.repo_dir,
            title="Validation Test Branch Diagram"
        )
        ...
        structure_content = create_git_branch_diagram(
            title="Validation Test Structure"
        )
...
class TestGitVisualizationWithSampleData:
    ...
    def test_custom_data_visualization(self):
        ...
        # Custom commit data
        commits = [
            {"hash": "a1b2c3d", "message": "Initial commit", "author": "Alice", "branch": "main", "date": "2024-01-01T10:00:00Z"},
            {"hash": "e4f5g6h", "message": "Add user authentication", "author": "Bob", "branch": "feature/user-auth", "date": "2024-01-05T14:30:00Z"},
            {"hash": "i7j8k9l", "message": "Fix security vulnerability", "author": "Charlie", "branch": "hotfix/security-fix", "date": "2024-01-10T09:15:00Z"},
            {"hash": "m1n2o3p", "message": "Update documentation", "author": "Alice", "branch": "develop", "date": "2024-01-12T16:45:00Z"},
            {"hash": "q4r5s6t", "message": "Refactor authentication module", "author": "Bob", "branch": "feature/user-auth", "date": "2024-01-15T11:20:00Z"}
        ]
        ...
        content = visualizer.visualize_git_tree_mermaid(
            branches=[{"name": b["name"], "created_at": "2024-01-01", "merged": True} for b in branches[:2]],
            commits=commits,
