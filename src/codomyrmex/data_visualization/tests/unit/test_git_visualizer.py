...
class TestGitVisualizer:
    ...
    def test_get_branch_color(self):
        ...
        assert self.visualizer._get_branch_color('main') == self.visualizer.colors['main']
        assert self.visualizer._get_branch_color('master') == self.visualizer.colors['main']
        assert self.visualizer._get_branch_color('develop') == self.visualizer.colors['develop']
        assert self.visualizer._get_branch_color('feature/test') == self.visualizer.colors['feature']
        assert self.visualizer._get_branch_color('hotfix/bug') == self.visualizer.colors['hotfix']
        assert self.visualizer._get_branch_color('release/v1.0') == self.visualizer.colors['release']
        assert self.visualizer._get_branch_color('unknown-branch') == self.visualizer.colors['commit']
    ...
    @patch('codomyrmex.data_visualization.git_visualizer.GIT_OPERATIONS_AVAILABLE', True)
    ...
    def test_visualize_git_tree_mermaid_with_repo_path(self, mock_current_branch, mock_commit_history, mock_is_repo):
    ...
    def test_visualize_repository_summary_png_with_custom_data(self, mock_plt):
        ...
        repo_data = {
            'status': {'clean': False, 'modified': ['file1.py'], 'untracked': ['file2.py']},
            'commits': [
                {'date': '2024-01-01T10:00:00Z', 'author_name': 'Alice', 'message': 'Initial commit'},
                {'date': '2024-01-02T11:00:00Z', 'author_name': 'Bob', 'message': 'Add feature'}
            ],
            'current_branch': 'main',
            'total_commits': 10
        }
    ...
    def test_create_comprehensive_git_report(self, mock_summary, mock_activity, mock_tree_mermaid, mock_tree_png, 
                                           mock_is_repo, mock_git_available):
        ...
        with patch.object(self.visualizer.mermaid_generator, 'create_git_workflow_diagram') as mock_workflow:
            mock_workflow.return_value = "workflow content"
            
            with patch.object(self.visualizer, '_get_repository_structure') as mock_structure:
                mock_structure.return_value = {"src": {"main.py": "file"}}
                
                with patch.object(self.visualizer.mermaid_generator, 'create_repository_structure_diagram') as mock_struct_diag:
...
class TestGitVisualizationErrorHandling:
    ...
    @patch('codomyrmex.data_visualization.git_visualizer.GIT_OPERATIONS_AVAILABLE', False)
