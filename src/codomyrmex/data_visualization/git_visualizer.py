...
try:
    from ..git_operations.git_manager import (
        get_commit_history, get_current_branch, get_status,
...
class GitVisualizer:
    ...
    def visualize_git_tree_png(
        ...
        try:
            ...
            for i, branch in enumerate(branches):
                ...
                # Add branch label
                ax.text(-0.5, y_pos, branch_name, fontweight='bold', 
            ...
            for i, commit in enumerate(commits[:max_commits]):
                ...
                ax.text(x_pos, y_pos - 0.15, commit.get('hash', ''), 
                       fontsize=8, ha='center', va='top')
                
                # Add commit message (rotated for space)
                message = commit.get('message', '')[:30]
                ax.text(x_pos, y_pos + 0.15, message, fontsize=8, 
            ...
            logger.error(f"Error creating Git tree PNG visualization: {e}", exc_info=True)
            return False
    ...
    def visualize_git_tree_mermaid(
        ...
        try:
            ...
            if repository_path and GIT_OPERATIONS_AVAILABLE:
                ...
                branches = [{'name': current_branch or 'main', 'created_at': '2024-01-01'}]
                commits = [
                    {
                        'hash': commit['hash'],
                        'message': commit['message'],
    ...
    def visualize_commit_activity_png(
        ...
        try:
            ...
            ax.text(0.02, 0.95, f'Total: {total_commits} commits\nAvg: {avg_commits:.1f} commits/day',
            ...
            logger.error(f"Error creating commit activity PNG chart: {e}", exc_info=True)
            return False
    ...
    def visualize_repository_summary_png(
        ...
        try:
            ...
            if any(status_data.values()):
                ax1.pie(status_data.values(), labels=status_data.keys(), autopct='%1.0f',
                       colors=['#90EE90', '#FFD700', '#FF6347'])
            ax1.set_title('Repository Status')
            
            # 2. Commit timeline (top-center and top-right)
            ...
            if commits:
                ...
                if dates:
                    ax2.plot(dates, range(len(dates)), 'o-', color=self.colors['commit'])
                    ax2.set_xlabel('Date')
                    ax2.set_ylabel('Commits')
                    ax2.set_title('Recent Commits Timeline')
                    plt.setp(ax2.xaxis.get_majorticklabels(), rotation=45)
            
            # 3. Author contributions (middle-left)
            ...
            if author_counts:
                ...
                ax3.barh(authors, counts, color=self.colors['feature'])
                ax3.set_title('Top Contributors')
                ax3.set_xlabel('Commits')
            
            # 4. Branch information (middle-center)
            ...
            ax4.text(0.5, 0.5, branch_info, transform=ax4.transAxes, ha='center', va='center',
                    fontsize=12, bbox=dict(boxstyle='round', facecolor=self.colors['main'], alpha=0.3))
            ax4.set_title('Branch Info')
            ax4.axis('off')
            
            # 5. Commit message word cloud data (middle-right)
            ...
            if commit_words:
                ...
                ax5.barh(words, counts, color=self.colors['develop'])
                ax5.set_title('Common Commit Words')
                ax5.set_xlabel('Frequency')
            
            # 6. Activity heatmap (bottom row)
            ...
            if commit_dates:
                ...
                if activity_matrix:
                    im = ax6.imshow(activity_matrix, cmap='Greens', aspect='auto')
                    ax6.set_title('Weekly Commit Activity Heatmap')
                    ax6.set_xlabel('Day of Week')
                    ax6.set_ylabel('Week')
                    ax6.set_xticks(range(7))
                    ax6.set_xticklabels(['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'])
                    ax6.set_yticks(range(len(weeks)))
                    ax6.set_yticklabels(weeks, fontsize=8)
                    
                    # Add colorbar
                    plt.colorbar(im, ax=ax6, label='Commits')
            ...
            logger.info(f"Repository summary PNG dashboard '{title}' generated successfully")
            return True
            
        except Exception as e:
            logger.error(f"Error creating repository summary PNG dashboard: {e}", exc_info=True)
            return False
    ...
    def create_comprehensive_git_report(
        ...
        results['git_tree_mermaid'] = bool(self.visualize_git_tree_mermaid(
            repository_path=repository_path,
        ...
        results['repo_summary'] = self.visualize_repository_summary_png(
            ...
            output_path=os.path.join(output_dir, f"{report_name}_summary_dashboard.png")
        )
        ...
        try:
            ...
            structure_content = self.mermaid_generator.create_repository_structure_diagram(
                repo_structure=repo_structure,
                title=f"Repository Structure - {os.path.basename(repository_path)}",
                output_path=os.path.join(output_dir, f"{report_name}_structure.mmd")
            )
        ...
        logger.info(f"Git report creation completed: {success_count}/{total_count} visualizations successful")
    ...
    def _generate_sample_commits(self, days_back: int = 30) -> List[Dict[str, Any]]:
        ...
        for i in range(days_back):
            ...
            commits.append({
                ...
                'date': date.isoformat(),
                'branch': 'main' if i % 3 == 0 else ('develop' if i % 3 == 1 else 'feature/sample')
            })
...
if __name__ == '__main__':
    ...
    if repo_path and os.path.exists(repo_path):
        ...
        logger.info(f"Sample Git tree Mermaid: {'Success' if mermaid_content else 'Failed'}")
    ...
    if not get_codomyrmex_logger('').hasHandlers():
        ...
        logger.info("Basic logging configured for direct script execution of git_visualizer.py.")
