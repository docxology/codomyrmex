...
from codomyrmex.git_operations.git_manager import (
    is_git_repository, get_current_branch, get_commit_history,
...
class RepositoryMetadata:
    ...
    last_metadata_update: str = field(default_factory=lambda: datetime.now(timezone.utc).isoformat())
...
class RepositoryMetadataManager:
    ...
    def __init__(self, metadata_file: Optional[str] = None, github_token: Optional[str] = None):
        """
        ...
        if metadata_file is None:
            metadata_file = os.path.join(os.path.dirname(__file__), "repository_metadata.json")
    ...
    def determine_access_level(self, github_data: Dict[str, Any], owner: str) -> AccessLevel:
    ...
    def update_local_repository_info(self, metadata: RepositoryMetadata) -> None:
        ...
        metadata.local_info.is_git_repo = is_git_repository(str(local_path)) if local_path.exists() else False
        
        if not metadata.local_info.is_git_repo:
            metadata.clone_status = CloneStatus.NOT_CLONED if not local_path.exists() else CloneStatus.ERROR
            return
        
        try:
            # Current branch
            metadata.local_info.current_branch = get_current_branch(str(local_path)) or ""
            ...
            if git_status and not git_status.get('error'):
                metadata.local_info.uncommitted_changes = not git_status.get('clean', True)
            ...
            # Total commits
            all_commits = get_commit_history(limit=1000, repository_path=str(local_path))
            metadata.local_info.total_local_commits = len(all_commits) if all_commits else 0
    ...
    def create_or_update_metadata(self, full_name: str, owner: str, name: str, 
                                  repo_type: str, url: str, description: str = "",
                                  local_path: str = "") -> RepositoryMetadata:
    ...
    def bulk_update_metadata(self, repositories: List[Dict[str, str]], 
        ...
        for repo_info in repositories:
            ...
            local_path = os.path.join(base_path, repo_info.get('local_path', '')) if base_path else ""
    ...
    def get_repositories_by_status(self, clone_status: CloneStatus) -> List[RepositoryMetadata]:
    ...
    def get_repositories_by_access(self, access_level: AccessLevel) -> List[RepositoryMetadata]:
    ...
    def get_outdated_repositories(self, days: int = 30) -> List[RepositoryMetadata]:
        ...
        for metadata in self.metadata.values():
            if metadata.last_sync_date:
                try:
                    last_sync = datetime.fromisoformat(metadata.last_sync_date.replace('Z', '+00:00'))
