...
from codomyrmex.git_operations.git_manager import (
    clone_repository, pull_changes, get_status, is_git_repository,
    get_current_branch, create_branch, switch_branch
)
from codomyrmex.git_operations.repository_metadata import (
    RepositoryMetadataManager, RepositoryMetadata, AccessLevel, CloneStatus
)
...
class RepositoryManager:
    ...
    def __init__(self, library_file: Optional[str] = None, base_path: Optional[str] = None,
                 metadata_file: Optional[str] = None, github_token: Optional[str] = None):
        """
        ...
        if library_file is None:
            library_file = os.path.join(os.path.dirname(__file__), "repository_library.txt")
        ...
        try:
            with open(self.library_file, 'r') as f:
                for line_num, line in enumerate(f, 1):
                    ...
                    parts = line.split('|')
                    if len(parts) != 6:
                        logger.warning(f"Invalid repository entry at line {line_num}: {line}")
                        continue
                    ...
                    try:
                        ...
                        logger.warning(f"Invalid repository type '{repo_type_str}' at line {line_num}")
                        continue
    ...
    def list_repositories(self, repo_type: Optional[RepositoryType] = None) -> List[Repository]:
        """
    ...
    def search_repositories(self, query: str) -> List[Repository]:
        ...
        for repo in self.repositories.values():
            if (query_lower in repo.name.lower() or 
                query_lower in repo.owner.lower() or 
    ...
    def clone_repository(self, full_name: str, custom_path: Optional[str] = None) -> bool:
        """
    ...
    def _setup_development_repo(self, repo_path: str, repo: Repository) -> None:
        ...
        try:
            ...
            logger.warning(f"Could not set up development branches for {repo.full_name}: {e}")
    
    def update_repository(self, full_name: str, custom_path: Optional[str] = None) -> bool:
        """
    ...
    def get_repository_status(self, full_name: str, custom_path: Optional[str] = None) -> Optional[Dict]:
        """
    ...
    def bulk_clone(self, repo_type: Optional[RepositoryType] = None, 
                   owner_filter: Optional[str] = None) -> Dict[str, bool]:
        """
    ...
    def bulk_update(self, repo_type: Optional[RepositoryType] = None,
                    owner_filter: Optional[str] = None) -> Dict[str, bool]:
        """
