...
from codomyrmex.git_operations.repository_metadata import (
    RepositoryMetadataManager, AccessLevel, CloneStatus, SyncStatus
)
...
def cmd_update_metadata(manager: RepositoryMetadataManager, args) -> None:
    ...
    if args.repository:
        ...
        repo_manager = RepositoryManager(
            library_file=args.from_library,
        ...
        results = manager.bulk_update_metadata(repositories, str(repo_manager.base_path))
...
def cmd_report(manager: RepositoryMetadataManager, args) -> None:
    ...
    if args.detailed:
        ...
        active_repos = [repo for repo in manager.metadata.values() 
    ...
    for repo in cloned_repos:
        try:
            ...
            if repo.local_info.uncommitted_changes:
                ...
                needs_sync.append((repo, f"{len(repo.local_info.untracked_files)} untracked files"))
            else:
...
def cmd_cleanup(manager: RepositoryMetadataManager, args) -> None:
    ...
    for full_name, metadata in manager.metadata.items():
        if metadata.local_path and not Path(metadata.local_path).exists():
            if args.dry_run:
                print(f"Would remove: {full_name} (path not found: {metadata.local_path})")
            else:
...
Examples:
  ...
  %(prog)s cleanup --dry-run
    ...
    update_parser.add_argument("--from-library", help="Update from repository library file")
    ...
    report_parser.add_argument("--detailed", action="store_true", help="Detailed report")
    report_parser.add_argument("--export", help="Export report to JSON file")
    report_parser.set_defaults(func=cmd_report)
    
    # Sync status command
    sync_parser = subparsers.add_parser("sync-status", help="Check synchronization status")
