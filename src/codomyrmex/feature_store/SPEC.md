...
| `set_value` | `feature_name, entity_id, value` | `None` | Store a value; auto-increments version. Validates value against definition. |
...
- `FeatureNotFoundError`: Raised when a feature definition or value is not found.
- `FeatureRegistrationError`: Raised when feature registration fails.
- `FeatureValidationError`: Raised when feature value validation fails.

## Dependencies

- **Internal**: `codomyrmex.exceptions`
- **External**: Standard library only (`threading`, `datetime`, `dataclasses`, `enum`, `logging`)
...
- `ingest_batch()` skips records without the `entity_id_field` key and logs a warning.
