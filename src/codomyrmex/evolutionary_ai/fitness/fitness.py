...
class FitnessFunction(ABC):
    ...
    @abstractmethod
    def evaluate(self, individual: Individual[Any]) -> FitnessResult:
...
class ScalarFitness(FitnessFunction):
    ...
    def evaluate(self, individual: Individual[Any]) -> FitnessResult:
        """Evaluate."""
        raw = float(self._fn(individual.genes))
...
class MultiObjectiveFitness(FitnessFunction):
    ...
    Args:
        objectives: A list of callables, each ``(genes) -> float``.
...
class ConstrainedFitness(FitnessFunction):
    ...
    Args:
        ...
        constraints: A list of callables ``(genes) -> float`` where a
    ...
    def evaluate(self, individual: Individual[Any]) -> FitnessResult:
        """Evaluate."""
        base_result = self._base.evaluate(individual)
        # Use primary value if scalar, or sum if vector (crude but simple for scalarised conversion)
        base_value = (
            base_result.value
        ...
        for constraint_fn in self._constraints:
            v = float(constraint_fn(individual.genes))
            violations.append(v)
            if v > 0:
                total_violation += v
