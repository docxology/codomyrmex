...
class RefactoringMixin:
                    ...
                    fixes["naming_convention_fixes"].append(
                        {
                            ...
                            "new_name": self._suggest_proper_name(
                                os.path.basename(violation.file_path)
                            ),
    ...
    def analyze_technical_debt(self) -> dict[str, Any]:
        ...
        try:
            ...
            complexity_suggestions = self.analyze_complexity_patterns()
            complexity_debt = (
                len(complexity_suggestions) * 4
            )  # 4 hours per complex function
            debt_analysis["debt_by_category"]["complexity"] = complexity_debt

            # Analyze dead code debt
            dead_code_findings = self.analyze_dead_code_patterns()
            dead_code_debt = (
                len([f for f in dead_code_findings if f.severity == "critical"]) * 1
            )  # 1 hour per critical dead code
            ...
            architecture_debt = (
                len(high_severity_violations) * 8
            )  # 8 hours per high-severity violation
            debt_analysis["debt_by_category"]["architecture"] = architecture_debt

            # Calculate total debt
            debt_analysis["total_debt_hours"] = sum(
                debt_analysis["debt_by_category"].values()
            )
    ...
    def _get_top_technical_debt_items(
        self, complexity_suggestions, dead_code_findings, architecture_violations
    ) -> list[dict[str, Any]]:
            ...
            debt_items.append(
                {
                    ...
                    "priority": "medium"
                    if suggestion.current_complexity < 25
                    else "high",
                }
            )
