...
- **Compromised Dependencies**: Downloading and installing malicious or compromised packages via `uv sync` or `npm install`.
...
- **Dependency Management**:
  - **Use Pinned Versions**: The root `pyproject.toml` / `package.json` files SHOULD use pinned, exact versions for all dependencies (e.g., `package==1.2.3` not `package>=1.2.0`). This mitigates the risk of inadvertently pulling in a newly compromised version of a sub-dependency.
  - **Hash-Checking**: Where feasible, use `uv pip install --require-hashes` to ensure that downloaded packages match expected checksums. (This is a project-wide goal).
...
- **Setup Scripts (if any)**:
  - Currently, the project primarily relies on `uv` and `npm` with `pyproject.toml` / `package.json`. `env_checker.py` is a Python script run directly.
...
Instead, please email **[blanket@activeinference.institute](mailto:blanket@activeinference.institute)** with the subject line: "**SECURITY Vulnerability Report: Environment Setup - Security Issue**".
...
- **Verify Sources**: When manually installing tools or dependencies not covered by the main `pyproject.toml`, ensure they are from trusted sources.
