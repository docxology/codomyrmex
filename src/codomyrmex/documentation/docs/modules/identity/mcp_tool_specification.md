...
2.  For each tool you define, copy the "Tool: `identity_tool`" section.
3.  Replace all bracketed placeholders (e.g., `identity_tool`, `[Brief description...]`, `[ParameterName]`, `[DataType]`, etc.) with specific details for your tool.
...
Persona level listing, creation, capability checks, and biocognitive verification via MCP
...
- **Return Format on Error**: Reiterate or specify the error object format (e.g., `{"error": "Specific error message related to identity_tool", "details": {...}}`).
...
---
<!-- Add more tool specifications below by copying the "Tool: `identity_tool`" section. --> 