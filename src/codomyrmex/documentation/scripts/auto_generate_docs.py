...
class AutoDocumentationGenerator:
    ...
    def find_modules(self, module_name: str | None = None) -> list[Path]:
...
"""
        ...
        if content.get("related_modules"):
            ...
            for rel in content["related_modules"]:
                agents_template += (
                    f"- **{rel['name']}** (`{rel['path']}`) - {rel['relationship']}\n"
                )
            agents_template += "\n"
...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help="Generate documentation for all modules"
    )
