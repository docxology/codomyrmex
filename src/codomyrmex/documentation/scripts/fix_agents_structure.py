...
def fix_agents_file(file_path):
    try:
        ...
        if (
            "## Operating Contracts" not in content
            and "# Operating Contracts" not in content
        ):
        ...
        if (
            "## Active Components" not in content
            and "# Active Components" not in content
        ):
            ...
            if "## Operating Contracts" in content:
                ...
                content = (
                    parts[0]
                    + get_active_components(file_path.parent)
                    + "\n## Operating Contracts"
                    + parts[1]
                )
            else:
