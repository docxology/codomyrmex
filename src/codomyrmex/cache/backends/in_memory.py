...
class InMemoryCache(Cache):
    ...
    def get(self, key: str) -> Any | None:
        ...
        self._stats.hits += 1
        # Move accessed item to end to maintain LRU order
    ...
    def set(self, key: str, value: Any, ttl: int | None = None) -> bool:
        ...
        if len(self._cache) >= self.max_size and key not in self._cache:
            # Remove oldest entry
            # Optimization: Use OrderedDict.popitem(last=False) for O(1) eviction instead of O(n) min() search. Reduces CPU overhead.
