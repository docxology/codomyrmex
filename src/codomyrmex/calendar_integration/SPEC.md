...
- `calendar_create_event(...)`: Creates a new event. Automatically injects `FristonBlanket@gmail.com` (default; override via `CODOMYRMEX_CALENDAR_ATTENDEE`) into the `attendees` array to guarantee bidirectional synchronization between PAI/agents and the primary calendar.
...
## 3. Google Calendar (`gcal/provider.py`) Implementation Details
