"""MCP tool definitions for the gemini coding agent module.

Exposes gemini single-turn chat execution as an MCP tool.
...
@mcp_tool(
    category="gemini",
    description=(
        "Execute a single-turn query against the gemini API. "
        "Useful for reasoning or code generation specific to gemini."
    ),
)
def gemini_execute(
    prompt: str,
    ...
    try:
        ...
        client = GeminiClient(config={"timeout": timeout})
