...
class CodebaseObserver:
    def __init__(self, model="llama3.2:latest"):
        ...
        self.session_id = "codomyrmex_sentinel_v1"
    ...
    def run_cycle(self):
        ...
        try:
            ...
            prompt = (
                ...
                f"{git_context}\n\n"
                "Analyze the trajectory of these changes. Identify architectural drift or technical debt. "
                "Surface exactly one high-impact recommendation, mentioning specific Tools or Skills to use."
            )
            
            print(f"[{datetime.datetime.now()}] Sentinel analyzing codebase via {self.client._ollama_model}...")
            
            # 3. Execute observation
            resp = self.client.chat_session(
                prompt=prompt, 
                session_id=self.session_id,
                # system_prompt=system_prompt # (Optional: set via set_system_prompt if needed)
                max_tokens=2048
            )
            ...
            rec_content = (
                ...
                f"{resp.content}\n\n"
                "---\n"
                f"*Generated by Hermes Sentinel (llama3.2) | Tool-Aware Mode | Length: {char_count} chars | Duration: {duration:.2f}s*\n"
            )
            
            filename.write_text(rec_content)
            print(f"[{datetime.datetime.now()}] Recommendation saved to {filename} ({char_count} chars, {duration:.2f}s)")
