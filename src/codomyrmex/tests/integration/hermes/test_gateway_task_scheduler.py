...
def test_client(tmp_path):
    ...
    return HermesClient(config={"hermes_session_db": str(db_path), "hermes_backend": "none"})
...
def test_hermes_create_and_update_task(test_client, monkeypatch) -> None:
    ...
    with SQLiteSessionStore(test_client._session_db_path) as store:
        ...
        assert session.metadata["workflow_tasks"]["task_1"]["result"] == "Fixture written successfully."
