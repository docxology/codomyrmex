...
- tool_module_info    -- importlib-based module introspection
- tool_pai_status     -- PAIBridge.get_status() wrapper
- tool_pai_awareness  -- DataProvider wrapper (may gracefully error)
- tool_list_workflows -- YAML-frontmatter workflow scanner
- tool_list_module_functions -- inspect-based function lister
- tool_get_module_readme     -- README/SPEC reader
"""
...
class TestToolModuleInfo:
    ...
    def test_known_module_returns_dict(self):
        ...
        result = tool_module_info(module_name="agents")
        assert isinstance(result, dict)
    ...
    def test_unknown_module_error_mentions_name(self):
        ...
        result = tool_module_info(module_name=module_name)
    ...
    def test_second_module_logging_monitoring(self):
        ...
        result = tool_module_info(module_name="logging_monitoring")
        assert isinstance(result, dict)
...
class TestToolPaiStatus:
    ...
    def test_no_exception_raised(self):
        """Calling tool_pai_status() must not raise any exception."""
        # If PAI is not installed, PAIBridge.get_status() should still return a valid dict
        try:
            result = tool_pai_status()
            assert isinstance(result, dict)
        except Exception as exc:
            pytest.fail(f"tool_pai_status() raised unexpectedly: {exc}")
...
class TestToolPaiAwareness:
    ...
    def test_no_exception_raised(self):
        """tool_pai_awareness() must never raise -- it catches ImportError/OSError."""
        try:
            result = tool_pai_awareness()
            assert isinstance(result, dict)
        except Exception as exc:
            pytest.fail(f"tool_pai_awareness() raised unexpectedly: {exc}")
...
class TestToolListWorkflows:
    ...
    def test_custom_project_root_missing(self):
        ...
        result = tool_list_workflows(project_root="/nonexistent/path/xyz")
...
class TestToolListModuleFunctions:
    ...
    def test_returns_dict(self):
        """Return value must be a dict."""
        result = tool_list_module_functions(module="logging_monitoring")
        assert isinstance(result, dict)
    ...
    def test_unknown_module_returns_error(self):
        ...
        result = tool_list_module_functions(module="nonexistent_module_xyz_123")
        assert "error" in result
...
class TestToolGetModuleReadme:
    ...
    def test_returns_dict(self):
        """Return value must be a dict."""
        result = tool_get_module_readme(module="agents")
        assert isinstance(result, dict)
    ...
    def test_unknown_module_returns_error(self):
        ...
        result = tool_get_module_readme(module="this_module_does_not_exist_xyz")
        assert isinstance(result, dict)
        assert "error" in result
