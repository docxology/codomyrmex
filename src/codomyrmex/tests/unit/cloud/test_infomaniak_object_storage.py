...
class TestInfomaniakObjectStorageClient:
    ...
    def test_list_containers(self, stub_openstack_connection):
        """list_containers returns container names from object_store."""
        c1 = Stub()
        c1.name = "alpha"
        c2 = Stub()
        c2.name = "beta"
        stub_openstack_connection.object_store.containers.return_value = [c1, c2]

        client = InfomaniakObjectStorageClient(stub_openstack_connection)
        result = client.list_containers()

        assert result == ["alpha", "beta"]

    def test_list_containers_error_returns_empty(self, stub_openstack_connection):
    ...
    def test_create_container(self, stub_openstack_connection):
        """create_container calls object_store.create_container and returns True."""
        client = InfomaniakObjectStorageClient(stub_openstack_connection)
        assert client.create_container("new-bucket") is True
        stub_openstack_connection.object_store.create_container.assert_called_once_with(
            "new-bucket"
        )

    def test_delete_container(self, stub_openstack_connection):
        """delete_container calls object_store.delete_container and returns True."""
        client = InfomaniakObjectStorageClient(stub_openstack_connection)
        assert client.delete_container("old-bucket") is True
        stub_openstack_connection.object_store.delete_container.assert_called_once_with(
            "old-bucket"
        )

    def test_get_container_metadata(self, stub_openstack_connection):
        """get_container_metadata returns dict from container.metadata."""
        stub_container = Stub()
        stub_container.metadata = {"X-Container-Read": ".r:*"}
        stub_openstack_connection.object_store.get_container_metadata.return_value = (
            stub_container
        )
    ...
    def test_get_container_metadata_error_returns_empty(
        ...
        stub_openstack_connection.object_store.get_container_metadata.side_effect = (
            Exception("not found")
        )
    ...
    def test_list_objects(self, stub_openstack_connection):
        """list_objects returns object names from a container."""
        obj1 = Stub()
        obj1.name = "file1.txt"
        obj2 = Stub()
        obj2.name = "dir/file2.txt"
        stub_openstack_connection.object_store.objects.return_value = [obj1, obj2]

        client = InfomaniakObjectStorageClient(stub_openstack_connection)
        result = client.list_objects("my-container", prefix="dir/")

        assert result == ["file1.txt", "dir/file2.txt"]
        stub_openstack_connection.object_store.objects.assert_called_once_with(
            "my-container", prefix="dir/"
        )

    def test_upload_object(self, stub_openstack_connection):
        ...
        stub_openstack_connection.object_store.upload_object.assert_called_once_with(
            container="bucket",
            name="key.txt",
            data=b"data bytes",
    ...
    def test_upload_file(self, stub_openstack_connection, tmp_path):
    ...
    def test_upload_file_error_returns_false(self, stub_openstack_connection):
    ...
    def test_download_object(self, stub_openstack_connection):
        """download_object returns raw bytes from object_store."""
        stub_openstack_connection.object_store.download_object.return_value = (
            b"hello swift"
        )
    ...
    def test_download_file(self, stub_openstack_connection, tmp_path):
    ...
    def test_download_file_no_data_returns_false(self, stub_openstack_connection):
        """download_file returns False when download_object returns None."""
        stub_openstack_connection.object_store.download_object.return_value = None
    ...
    def test_delete_object(self, stub_openstack_connection):
        """delete_object calls object_store.delete_object with correct kwargs."""
        client = InfomaniakObjectStorageClient(stub_openstack_connection)
        assert client.delete_object("bucket", "key.txt") is True

        stub_openstack_connection.object_store.delete_object.assert_called_once_with(
            "key.txt", container="bucket"
        )

    def test_get_object_metadata(self, stub_openstack_connection):
        """get_object_metadata returns structured metadata dict."""
        stub_obj = Stub()
        stub_obj.name = "file.txt"
        stub_obj.content_length = 1024
        stub_obj.content_type = "text/plain"
        stub_obj.etag = "abc123"
        stub_obj.last_modified_at = "2026-01-15T10:00:00Z"
        stub_openstack_connection.object_store.get_object_metadata.return_value = (
            stub_obj
        )
    ...
    def test_set_container_read_acl(self, stub_openstack_connection):
        ...
        stub_openstack_connection.object_store.set_container_metadata.assert_called_once_with(
            "public-bucket", read_acl=".r:*"
        )

    def test_set_container_write_acl(self, stub_openstack_connection):
...
class TestInfomaniakS3Client:
    ...
    def test_list_buckets(self, stub_s3_client):
    ...
    def test_create_bucket(self, stub_s3_client):
        """create_bucket calls S3 create_bucket and returns True."""
        client = InfomaniakS3Client(stub_s3_client)
        assert client.create_bucket("new-bucket") is True
        stub_s3_client.create_bucket.assert_called_once_with(Bucket="new-bucket")

    def test_delete_bucket(self, stub_s3_client):
        """delete_bucket calls S3 delete_bucket and returns True."""
        client = InfomaniakS3Client(stub_s3_client)
        assert client.delete_bucket("old-bucket") is True
        stub_s3_client.delete_bucket.assert_called_once_with(Bucket="old-bucket")

    def test_bucket_exists_true(self, stub_s3_client):
        """bucket_exists returns True when head_bucket succeeds."""
        stub_s3_client.head_bucket.return_value = {}

        client = InfomaniakS3Client(stub_s3_client)
        assert client.bucket_exists("existing") is True

    def test_bucket_exists_false(self, stub_s3_client):
        """bucket_exists returns False when head_bucket raises."""
        stub_s3_client.head_bucket.side_effect = Exception("404 Not Found")
    ...
    def test_list_objects(self, stub_s3_client):
        """list_objects extracts keys from S3 Contents response."""
        stub_s3_client.list_objects_v2.return_value = {
        ...
        assert result == ["k1", "k2"]
        stub_s3_client.list_objects_v2.assert_called_once_with(
            Bucket="bucket", MaxKeys=1000, Prefix="docs/"
        )

    def test_upload_file(self, stub_s3_client):
        """upload_file delegates to S3 upload_file."""
        client = InfomaniakS3Client(stub_s3_client)
        result = client.upload_file("bucket", "key.txt", "/tmp/claude/file.txt")

        assert result is True
        stub_s3_client.upload_file.assert_called_once_with(
            "/tmp/claude/file.txt", "bucket", "key.txt", ExtraArgs={}
        )

    def test_upload_data(self, stub_s3_client):
        ...
        assert result is True
        stub_s3_client.put_object.assert_called_once_with(
            Bucket="bucket",
            Key="key.json",
            Body=b'{"a":1}',
    ...
    def test_download_file(self, stub_s3_client):
        """download_file calls S3 download_file and returns True."""
        client = InfomaniakS3Client(stub_s3_client)
        result = client.download_file("bucket", "key.txt", "/tmp/claude/out.txt")

        assert result is True
        stub_s3_client.download_file.assert_called_once_with(
            "bucket", "key.txt", "/tmp/claude/out.txt"
        )

    def test_download_data(self, stub_s3_client):
        """download_data reads Body from S3 get_object response."""
        stub_body = Stub()
        stub_body.read.return_value = b"raw bytes"
        stub_s3_client.get_object.return_value = {"Body": stub_body}
    ...
    def test_generate_presigned_url_get(self, stub_s3_client):
        """generate_presigned_url with GET uses get_object ClientMethod."""
        stub_s3_client.generate_presigned_url.return_value = (
            "https://s3.example.com/signed"
        )

        client = InfomaniakS3Client(stub_s3_client)
        url = client.generate_presigned_url(
            "bucket", "key.txt", expires_in=3600, operation="get_object"
        )

        assert url == "https://s3.example.com/signed"
        stub_s3_client.generate_presigned_url.assert_called_once_with(
            ClientMethod="get_object",
            Params={"Bucket": "bucket", "Key": "key.txt"},
            ExpiresIn=3600,
        )

    def test_generate_presigned_url_put(self, stub_s3_client):
        ...
        client = InfomaniakS3Client(stub_s3_client)
        url = client.generate_presigned_url("bucket", "key.txt", operation="put_object")
    ...
    def test_copy_object(self, stub_s3_client):
        """copy_object calls S3 copy_object with CopySource dict."""
        client = InfomaniakS3Client(stub_s3_client)
        result = client.copy_object("src-bucket", "src-key", "dst-bucket", "dst-key")

        assert result is True
        stub_s3_client.copy_object.assert_called_once_with(
            Bucket="dst-bucket",
            Key="dst-key",
            CopySource={"Bucket": "src-bucket", "Key": "src-key"},
        )

    def test_list_objects_paginated(self, stub_s3_client):
        """list_objects_paginated uses paginator to collect all keys."""
        stub_paginator = Stub()
        stub_paginator.paginate.return_value = [
            {"Contents": [{"Key": "page1-k1"}, {"Key": "page1-k2"}]},
            {"Contents": [{"Key": "page2-k1"}]},
        ]
        stub_s3_client.get_paginator.return_value = stub_paginator

        client = InfomaniakS3Client(stub_s3_client)
        result = client.list_objects_paginated("bucket", prefix="data/")

        assert result == ["page1-k1", "page1-k2", "page2-k1"]
        stub_s3_client.get_paginator.assert_called_once_with("list_objects_v2")
        stub_paginator.paginate.assert_called_once_with(Bucket="bucket", Prefix="data/")

    def test_delete_objects_batch(self, stub_s3_client):
        """delete_objects_batch sends batch delete and returns deleted count and errors."""
        stub_s3_client.delete_objects.return_value = {
    ...
    def test_enable_versioning(self, stub_s3_client):
        """enable_versioning calls put_bucket_versioning with Enabled status."""
        client = InfomaniakS3Client(stub_s3_client)
        assert client.enable_versioning("bucket") is True

        stub_s3_client.put_bucket_versioning.assert_called_once_with(
            Bucket="bucket",
            VersioningConfiguration={"Status": "Enabled"},
        )

    def test_get_versioning(self, stub_s3_client):
        """get_versioning returns the Status string from the response."""
        stub_s3_client.get_bucket_versioning.return_value = {"Status": "Enabled"}

        client = InfomaniakS3Client(stub_s3_client)
        assert client.get_versioning("bucket") == "Enabled"

    def test_get_bucket_policy(self, stub_s3_client):
        """get_bucket_policy returns the policy JSON string."""
        stub_s3_client.get_bucket_policy.return_value = {
    ...
    def test_get_bucket_policy_no_policy_returns_none(self, stub_s3_client):
        """get_bucket_policy returns None for NoSuchBucketPolicy error."""
        stub_s3_client.get_bucket_policy.side_effect = Exception(
    ...
    def test_put_bucket_policy(self, stub_s3_client):
        ...
        stub_s3_client.put_bucket_policy.assert_called_once_with(
...
class TestInfomaniakSwiftClientExpanded:
    ...
    def _make_client(self):
        ...
        stub_conn = Stub()
        return InfomaniakObjectStorageClient(connection=stub_conn), stub_conn
...
class TestInfomaniakS3ClientExpanded:
    ...
    def _make_client(self):
        ...
        stub_s3 = Stub()
        return InfomaniakS3Client(client=stub_s3), stub_s3
    ...
    def test_upload_file(self):
        ...
        s3.upload_file.assert_called_once_with(
            "/tmp/f.txt", "bkt", "key.txt", ExtraArgs={}
        )
