...
FORBIDDEN_CLAIMS = {
    ...
    re.compile(
        r"\b1,191\b"
    ): "docs inventory is currently 1,195 Markdown files under docs/",
    re.compile(
        r"\b1,192\b"
    ): "docs inventory is currently 1,195 Markdown files under docs/",
    re.compile(
...
def test_manuscript_generator_uses_branch_coverage_contract() -> None:
    generator = _read("src/manuscript_variables.py")
...
def test_falsification_figure_uses_canonical_attack_vector_names_and_severity() -> None:
    figures = _read("scripts/generate_manuscript_figures.py")
...
def test_manuscript_artifact_count_sources_match_documented_surfaces() -> None:
    ...
    module_path = REPO_ROOT / "src" / "manuscript_variables.py"
    spec = importlib.util.spec_from_file_location(
