...
def conflicted_repo(tmp_path):
    ...
    subprocess.run(
        ["git", "checkout", "main"], cwd=tmp_path, check=True, capture_output=True
    )
    ...
    subprocess.run(
        ["git", "merge", "--no-ff", "feature"], cwd=tmp_path, capture_output=True
    )
...
def trivial_conflict_repo(tmp_path):
    ...
    subprocess.run(["git", "add", "."], cwd=tmp_path, check=True, capture_output=True)
    subprocess.run(
        ["git", "commit", "-m", "feature whitespace"],
        cwd=tmp_path,
        check=True,
        capture_output=True,
    )
...
class TestConflictBlock:
    ...
    def test_resolved_defaults_to_false(self):
        block = ConflictBlock(
            file_path="a.py", start_line=1, ours_content="a", theirs_content="b"
        )
...
class TestMergeResolverResolveFile:
    ...
    def test_resolve_file_already_resolved_returns_true(self, clean_git_repo):
        ...
        subprocess.run(
            ["git", "add", "."], cwd=clean_git_repo, check=True, capture_output=True
        )
        subprocess.run(
