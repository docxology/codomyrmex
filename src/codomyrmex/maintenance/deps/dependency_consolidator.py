...
def parse_requirements_file(file_path: Path) -> list[tuple[str, str, str]]:
    ...
    for line in content.splitlines():
        ...
        match = re.match(
            r"^([a-zA-Z0-9_-]+[a-zA-Z0-9._-]*)([<>=!~=]+)?([0-9.]+)?", line
        )
        if match:
...
def analyze_dependencies(root: Path) -> dict[str, dict]:
    ...
    dependencies = defaultdict(
        lambda: {"versions": set(), "sources": [], "conflicts": []}
    )
...
def generate_pyproject_additions(
    dependencies: dict[str, dict], pyproject_content: str
...
def main():
    ...
    # Generate optional dependencies section
    optional_deps, module_mapping = generate_pyproject_additions(
...
Generated by: `src/codomyrmex/maintenance/dependency_consolidation_report.md`
...
"""
    ...
    if conflicts:
        for package_name, info in conflicts.items():
            ...
            report_content += (
                f"- Conflicting versions: {', '.join(info['conflicts'])}\n"
            )
