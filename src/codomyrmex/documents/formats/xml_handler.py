...
def read_xml(file_path: str | Path, encoding: str | None = None) -> str:
    ...
    try:
        ...
        raise DocumentReadError(
            f"Invalid XML: {str(e)}", file_path=str(file_path)
        ) from e
    except Exception as e:
        logger.error(f"Error reading XML file {file_path}: {e}")
        raise DocumentReadError(
            f"Failed to read XML file: {str(e)}", file_path=str(file_path)
        ) from e
...
def write_xml(content: str, file_path: str | Path, encoding: str | None = None) -> None:
    ...
    try:
        ...
        raise DocumentWriteError(
            f"Invalid XML content: {str(e)}", file_path=str(file_path)
        ) from e
    except Exception as e:
        logger.error(f"Error writing XML file {file_path}: {e}")
        raise DocumentWriteError(
            f"Failed to write XML file: {str(e)}", file_path=str(file_path)
        ) from e
