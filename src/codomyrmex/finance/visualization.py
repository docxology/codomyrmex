...
if TYPE_CHECKING:
    from .ledger.ledger import Account

from .ledger.ledger import AccountType
...
def group_by_type(accounts: list[Account]) -> dict[str, list[Account]]:
    ...
    for acct in accounts:
        groups[acct.account_type.value].append(acct)
    return dict(groups)
...
def income_statement_text(accounts: list[Account]) -> str:
    ...
    for acct in revenue_accts:
        ...
        total_rev += Decimal(str(acct.balance))
    ...
    for acct in expense_accts:
        lines.append(f"  {acct.name:<28} {acct.balance:>10,.2f}")
        total_exp += Decimal(str(acct.balance))
...
def balance_sheet_text(accounts: list[Account]) -> str:
    ...
    for acct in assets:
        lines.append(f"  {acct.name:<28} {acct.balance:>10,.2f}")
        total_a += Decimal(str(acct.balance))
    lines.append(f"  {'Total Assets':<28} {total_a:>10,.2f}")

    lines.append("")
    lines.append("LIABILITIES:")
    total_l = Decimal("0.00")
    for acct in liabilities:
        lines.append(f"  {acct.name:<28} {acct.balance:>10,.2f}")
        total_l += Decimal(str(acct.balance))
    ...
    for acct in equity:
        lines.append(f"  {acct.name:<28} {acct.balance:>10,.2f}")
        total_e += Decimal(str(acct.balance))
