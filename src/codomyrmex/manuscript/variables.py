...
def _count_colony_kernel_mcp_tools(colony_kernel_dir: Path, fallback: int) -> int:
...
def _run_colony_kernel_coverage(project_root: Path, test_path: Path) -> dict[str, Any]:
    coverage_path = project_root / "output" / "data" / "colony_kernel_coverage.json"
    # If cached coverage exists and is fresh (within 1 hour), reuse it
    if coverage_path.exists() and (time.time() - coverage_path.stat().st_mtime) < 3600:
        try:
            ...
            if pct is not None:
                ...
                return {"collected": count, "coverage_pct": round(float(pct), 1)}
        except Exception:
            pass
    ...
    output = f"{result.stdout}\n{result.stderr}".strip()
    if result.returncode != 0 and not coverage_path.exists() and not result.stdout:
        raise RuntimeError(
...
def compute_variables(
    ...
    paper_title: str = str(paper.get("title", "Codomyrmex"))
    paper_subtitle: str = str(paper.get("subtitle", ""))
    publication_date: str = _publication_date(paper.get("date"))
    publication_date_display: str = _display_date(publication_date)
    publication: dict[str, Any] = config.get("publication", {})
    doi_value: str = str(
        publication.get("doi") or publication.get("doi_status") or "forthcoming"
    )
    github_repository: str = str(
        publication.get("github_repository", "docxology/codomyrmex")
    )
    ...
    gate_execute_threshold: float = float(
        experiment.get("gate_execute_threshold", 0.75)
    )
    ...
    trust_promote_threshold: float = float(
        experiment.get("trust_promote_threshold", 0.20)
    )

    colony_signal_types: list[str] = experiment.get(
        "colony_signal_types",
        [
            "FAILURE",
            "SUCCESS",
    ...
    decay_rates_list: list[str] = experiment.get(
        "decay_rates", ["FAST", "NORMAL", "SLOW"]
    )
    ...
    gate_score_weights: dict[str, float] = experiment.get(
        "gate_score_weights",
        {
            "budget": 0.30,
            "risk": 0.30,
    ...
    gate_weight_completeness: float = float(
        gate_score_weights.get("completeness", 0.15)
    )

    budget_dimensions: list[str] = experiment.get(
        "budget_dimensions",
        [
            "llm_calls",
            "runtime_seconds",
    ...
    pheromone_retention_slow: float = round(
        math.exp(-base_evaporation_rate * decay_rate_slow), 3
    )  # e^(-0.02) ≈ 0.980
    pheromone_retention_normal: float = round(
        math.exp(-base_evaporation_rate * decay_rate_normal), 3
    )  # e^(-0.10) ≈ 0.905
    pheromone_retention_fast: float = round(
        math.exp(-base_evaporation_rate * decay_rate_fast), 3
    )  # e^(-0.30) ≈ 0.741
    ...
    pheromone_fast_loss_8_tick_pct: int = round(
        (1 - pheromone_retention_fast**8) * 100
    )  # ≈ 91
    pheromone_slow_retention_8_tick_pct: int = round(
        pheromone_retention_slow**8 * 100
    )  # ≈ 85
    ...
    agent_roles: list[str] = experiment.get(
        "agent_roles",
        [
            "SANDBOX",
            "REPAIR_ANT",
    ...
    first_author_orcid: str = (
        authors_list[0].get("orcid", "forthcoming") if authors_list else "forthcoming"
    )
    keywords_str: str = (
        ", ".join(keywords_list) if keywords_list else "ai-agents, mcp, multi-agent"
    )
    ...
    if ruff_result.returncode not in (0, 1):
        raise RuntimeError(
            "ruff check failed while computing manuscript metrics "
    ...
    if ty_result.returncode not in (0, 1):
        raise RuntimeError(
            "ty check failed while computing manuscript metrics "
    ...
    ck_files: int = (
        _count_python_files(colony_kernel_dir) if colony_kernel_dir.is_dir() else 10
    )
    if ck_files == 0:
        ck_files = 10
    ...
    mcp_tools_artifact: int = _count_colony_kernel_mcp_tools(
        colony_kernel_dir, mcp_tool_count
    )
    ...
    variables: dict[str, str] = {
        ...
        "RESULT_PHEROMONE_SLOW_RETENTION_8_TICK_PCT": str(
            pheromone_slow_retention_8_tick_pct
        ),
