    ...
    def benchmark_model(
        ...
        for i, prompt in enumerate(test_prompts):
            ...
            benchmark_data = {
                ...
                "tokens_per_second": (
                    len(result.response) / result.execution_time
        ...
        benchmark_summary = {
            ...
            "avg_execution_time": (
                sum(r["execution_time"] for r in successful_runs) / len(successful_runs)
                if successful_runs
                else 0
            ),
            "avg_tokens_per_second": (
                sum(r["tokens_per_second"] for r in successful_runs)
                / len(successful_runs)
                if successful_runs
                else 0
            ),
