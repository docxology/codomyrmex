...
class CompositionVisualizer:
    ...
    def create_analysis_overview_dashboard(
        ...
        if "status_distribution" in analysis_summary:
            ...
            ax1.set_title(
                "Pattern Status", fontsize=self.theme.font.label_size, fontweight="bold"
            )
        ...
        if "key_metrics" in analysis_summary:
            ...
            ax5.set_title(
                "Key Metrics", fontsize=self.theme.font.label_size, fontweight="bold"
            )
        ...
        if "part_distribution" in analysis_summary:
            ...
            ax6.bar(
                parts, counts, color=colors, alpha=0.8, edgecolor="black", linewidth=1.0
            )
    ...
    def create_pattern_landscape_map(
        ...
        if pattern_metadata:
            colors = [
                self.theme.get_status_color(
                    pattern_metadata.get(pid, {}).get("status", "Stub")
                )
        ...
        if pattern_metadata:
            ...
            legend_elements = [
                Patch(
                    facecolor=self.theme.get_status_color(s),
                    edgecolor="black",
                    label=s,
                    alpha=0.7,
                )
