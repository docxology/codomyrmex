...
class StatusReporter:
    ...
    def check_project_structure(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "docs_dir": (self.project_root / "src" / "codomyrmex" / "documentation").exists(),
    ...
    def _check_virtual_env_dir(self) -> bool:
        ...
        return any((self.project_root / candidate).exists() for candidate in venv_candidates)
    ...
    def check_git_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                status["staged_changes"] = sum(1 for line in lines if line.startswith(('A ', 'M ', 'D ')))
                status["unstaged_changes"] = sum(1 for line in lines if line.endswith(('M', 'D', '??')))
    ...
    def _display_python_status(self, env_status: Dict[str, Any]) -> None:
        ...
        if env_status['virtual_env']:
            ...
            print(self.format_message("   Virtual Environment: Not detected", "warning"))
    ...
    def _display_project_structure(self, structure: Dict[str, Any]) -> None:
        ...
        for name, exists in status_items:
            status = self.format_message("✅", "success") if exists else self.format_message("❌", "error")
            print(f"   {status} {name}")
    ...
    def _display_dependencies_status(self, deps: Dict[str, Any]) -> None:
        ...
        print(f"\n📦 Dependencies:")
        print(f"   Available: {deps['available_count']}/{deps['total_count']} ({deps['success_rate']:.1f}%)")
        ...
        for group_name, dep_list in groups:
            ...
            for dep in dep_list:
                if dep in deps["dependencies"]:
                    ...
                    status = self.format_message("✅", "success") if available else self.format_message("❌", "error")
                    print(f"      {status} {dep}")
    ...
    def export_report(self, filename: Optional[str] = None) -> str:
        ...
        try:
            ...
            print(self.format_message(f"Status report exported to: {output_path}", "success"))
