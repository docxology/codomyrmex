...
def _cognilayer_db_path() -> Path:
    """Resolve DB path from the public ``cognilayer_bridge`` shim (tests patch that module)."""
    import codomyrmex.agentic_memory.cognilayer_bridge as _pub

    return _pub.COGNILAYER_DB


def _get_db_connection() -> sqlite3.Connection:
    """Open a read/write connection to the CogniLayer SQLite database."""
    db = _cognilayer_db_path()
    if not db.exists():
        raise FileNotFoundError(
            f"CogniLayer database not found at {db}. "
...
def get_memory_stats() -> dict[str, Any]:
    ...
    db = _cognilayer_db_path()
    if not db.exists():
        return {"installed": False, "error": f"Database not found: {db}"}
