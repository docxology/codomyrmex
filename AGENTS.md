...
- **Zero-Mock Policy:** All tests must use real components. No mocks. Narrow `monkeypatch.setenv`/`delenv`/`chdir` and `tmp_path` fixtures are permitted for test-input isolation — see [docs/development/testing-strategy.md § Zero-Mock Policy (clarified)](docs/development/testing-strategy.md#zero-mock-policy-clarified) and [.github/CONTRIBUTING.md § Testing Requirements](.github/CONTRIBUTING.md#testing-requirements).
...
- **Generated leaf docs:** Thousands of per-folder `AGENTS.md` / `README.md` files are produced by tooling, not by hand.
  ...
  - **Module docs (`docs/modules/<name>/`):** preview a source-derived refresh with `uv run --locked python scripts/documentation/enrich_module_docs.py --repo-root . --dry-run --module <name>`. Applying changes requires explicit `--apply`; replacing an existing unmarked file additionally requires the matching `--force-readmes`, `--force-agents`, or `--force-specs` flag. Curated README/AGENTS markers remain protected even under force. Never run a repository-wide apply during the hand-pass freeze.
  - **QA:** `make docs-check` is authoritative and read-only apart from reports/build output. It runs the scoped RASP pair check, the package-wide README/AGENTS command/link audit, repository link/content/structure checks, triple-check, and strict MkDocs. Standalone receipts: `uv run --locked python scripts/documentation/audit_readme_agents.py --repo-root . --strict` and `uv run --locked python src/codomyrmex/documentation/scripts/triple_check.py --repo-root . --fail-on-issues`.
...
This project is indexed by GitNexus as **codomyrmex** (207963 symbols, 302104 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
