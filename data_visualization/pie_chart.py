...
def create_pie_chart(
    ...
    if not labels or not sizes:
        logger.warning("Empty data for labels or sizes in pie chart. No plot generated.")
        return
    if len(labels) != len(sizes):
        logger.warning(f"Length mismatch for pie chart: labels ({len(labels)}) vs sizes ({len(sizes)}). Plot not generated.")
        return
    if explode and len(explode) != len(labels):
        logger.warning(f"Length mismatch for pie chart explode: labels ({len(labels)}) vs explode ({len(explode)}). Ignoring explode.")
        explode = None
    ...
    if sum(s for s in sizes if isinstance(s, (int, float))) == 0:
        logger.warning(f"All sizes are zero for pie chart '{title}'. Plotting with equal segments if labels exist, but percentages might be misleading.")
        # Optionally, plot equal segments or skip autopct
        # sizes = [1] * len(labels) # Plot equal segments
        # autopct = None

    ax.pie(sizes, explode=explode, labels=labels, autopct=autopct, shadow=True, startangle=startangle)
...
if __name__ == '__main__':
    ...
    if not get_codomyrmex_logger('').hasHandlers():
        ...
        logger.info("Basic logging configured for direct script execution of pie_chart.py.") 