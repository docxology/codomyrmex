...
def create_bar_chart(
    ...
    if not categories or not values:
        logger.warning("Empty data for categories or values in bar chart. No plot generated.")
        return
    if len(categories) != len(values):
        logger.warning(f"Length mismatch for bar chart: categories ({len(categories)}) vs values ({len(values)}). Plot not generated.")
        return
...
if __name__ == '__main__':
    ...
    logger.info(f"Vertical bar chart example saved to {output_dir / 'vertical_bar_chart.png'}")
    ...
    logger.info(f"Horizontal bar chart example saved to {output_dir / 'horizontal_bar_chart.png'}")

    import logging
    if not get_codomyrmex_logger('').hasHandlers():
        logging.basicConfig(level=logging.DEBUG, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        logger.info("Basic logging configured for direct script execution of bar_chart.py.") 