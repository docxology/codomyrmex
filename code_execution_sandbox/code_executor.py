...
def run_code_in_docker(
    ...
    container_cmd = [cmd.format(filename=code_file_path) for cmd in language_config["command"]]
    ...
    docker_cmd = ["docker", "run", "--rm"] + docker_args + [language_config["image"]] + container_cmd
    ...
    try:
        ...
        try:
            ...
            try:
                container_id_cmd = ["docker", "ps", "-q", "--filter", f"ancestor={language_config['image']}"]
                container_ids = subprocess.check_output(container_id_cmd, universal_newlines=True).strip().split('\n')
