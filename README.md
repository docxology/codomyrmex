...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-36%2C028-brightgreen?style=flat-square" alt="Tests">
</p>
...
Codomyrmex is a modular library of **130 top-level modules** under `src/codomyrmex/` spanning AI agents, cloud infrastructure, security, finance, multimedia, and more. The count is honest (every directory has an `__init__.py`) — roughly **25 modules are substantial production surfaces** (agents, llm, cloud, security, coding, documentation, orchestrator, etc.), **~75 are legitimate but narrower utilities**, and **~30 are teaching/exploratory modules** (dpo, nas, ssm, softmax_opt, etc.) that demonstrate techniques rather than ship production workflows. A documented **Zero-Mock** policy governs the active test suites, while the measured quality gates and inventory describe what was actually checked in a given run; they do not imply that every method is complete or production-safe. In the complete locked dependency profile (`--all-groups --all-extras`), the ecosystem exposes **612** runtime MCP tools in the generated manifest and **627** production `@mcp_tool` decorator lines in Python sources for Claude, Gemini, GPT, and any Model Context Protocol client ([docs/reference/inventory.md](docs/reference/inventory.md)). It includes **3,000+ Python files**, **36,028** collected tests (`uv run --locked --all-groups --all-extras python scripts/doc_inventory.py --pytest`), **1,204** Markdown files under `docs/`, and **37** GitHub Actions workflows (`.github/workflows/*.yml`). These are measured snapshot values for that dependency profile; refresh the linked inventory before treating them as current.

## Technical report

The source-bound Codomyrmex v1.3.0 technical report is archived as a Zenodo record family: [concept DOI 10.5281/zenodo.21750800](https://doi.org/10.5281/zenodo.21750800), [version DOI 10.5281/zenodo.21750801](https://doi.org/10.5281/zenodo.21750801), and [GitHub release `v1.3.0-paper`](https://github.com/docxology/codomyrmex/releases/tag/v1.3.0-paper). The report documents implementation contracts and controlled fixtures; it does not claim external effectiveness, calibration, or production safety.
...
- [GitHub Actions Workflows](#️-github-directory-overview)
- [License](#-license)
...
| 🧪 **36,028 Collected Tests** | Complete locked dependency profile; `uv run --locked --all-groups --all-extras python scripts/doc_inventory.py --pytest`; zero-mock policy |
...
| [**docs/modules/**](docs/modules/) | 130 pkgs | Per-module documentation (README, SPEC, AGENTS, PAI per module) |
...
sequenceDiagram
    ...
    MCP->>Tools: Discover 612 runtime tools (complete dependency profile; 627 source decorators; 130 top-level modules)
...
| **Collected tests** | 36,028 in the complete locked dependency profile (`uv run --locked --all-groups --all-extras python scripts/doc_inventory.py --pytest`) |
...
| **Testing Policy** | Zero-Mock for active suites; scope and exclusions are reported by each gate |
...
<p align="center">
  ...
  <sub>130 modules · 612 runtime MCP tools · 627 decorators · 36,028 tests · 1,204 docs · 37 workflows · Zero-Mock policy · Evidence-scoped</sub>
</p>
