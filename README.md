...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/MCP_Runtime-611-orange?style=for-the-badge" alt="MCP runtime tools">
  <img src="https://img.shields.io/badge/Workflows-38-purple?style=for-the-badge" alt="Workflows">
  ...
  <img src="https://img.shields.io/badge/tests-35%2C225-brightgreen?style=flat-square" alt="Tests">
</p>
...
Codomyrmex is a production-grade library of **130 top-level modules** under `src/codomyrmex/` spanning AI agents, cloud infrastructure, security, finance, multimedia, and more — all built on a strict **Zero-Mock** testing policy ensuring every method is real, tested, documented, and functional. The ecosystem exposes **611** runtime MCP tools in the generated manifest and **626** production `@mcp_tool` decorator lines in Python sources for Claude, Gemini, GPT, and any Model Context Protocol client ([docs/reference/inventory.md](docs/reference/inventory.md)). It includes **3,000+ Python files**, **35,225** collected tests (`uv run python scripts/doc_inventory.py --pytest`), **1,204 Markdown** files under `docs/`, and **38** GitHub Actions workflows (`.github/workflows/*.yml`).
...
| | |
|---|---|
| 🧩 **130 Top-Level Modules** | Packages under `src/codomyrmex/` — composable, independently versioned, zero-mock tested |
| 🤖 **611 Runtime MCP Tools** | 626 source `@mcp_tool` decorator lines; see [inventory](docs/reference/inventory.md) |
| 🧪 **35,225 Collected Tests** | `uv run python scripts/doc_inventory.py --pytest`; zero-mock policy |
...
sequenceDiagram
    ...
    MCP->>Tools: Discover 611 runtime tools (626 source decorators; 130 top-level modules)
...
```text
codomyrmex/
├── .github/                  # 38 GitHub Actions workflows, templates, docs
...
| **Collected tests** | 35,225 (`uv run python scripts/doc_inventory.py --pytest`) |
| **Documentation Files** | 1,204 Markdown under `docs/` (`find docs -name '*.md'`) |
| **GitHub Workflows** | 38 (`.github/workflows/*.yml`) |
| **MCP Runtime Tools** | 611 (generated runtime manifest) |
| **MCP Tool Decorators** | 626 (`@mcp_tool` lines, production tree) |
...
<p align="center">
  ...
  <sub>130 modules · 611 runtime MCP tools · 626 decorators · 35,225 tests · 1,204 docs · 38 workflows · Zero-Mock · Production-Grade</sub>
</p>
