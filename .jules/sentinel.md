...
## 2024-05-18 - Missing Hardcoded Password Detection

**Vulnerability:** The secret scanner lacked a pattern to detect generic hardcoded passwords (e.g., `password="mySuperSecretPassword123!"` <!-- pragma: allowlist secret -->), potentially allowing common credential formats to bypass detection.
**Learning:** Relying solely on platform-specific tokens (like Stripe or AWS keys) leaves a significant gap, as many systems still use generic username/password authentication.
**Prevention:** Ensured the secret scanner includes robust regex patterns to catch generic password assignments in code.
